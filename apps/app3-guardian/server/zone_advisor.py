...
interactive prompt:
  ...
  python3 zone_advisor.py --url http://127.0.0.1:11434 --model gemma3:4b
  python3 zone_advisor.py --provider openai --url https://api.example.com/v1 --model gemma --api-key sk-...
...
# Optional local script configuration. Keep secrets in env vars or CLI flags.
SCRIPT_LLM_PROVIDER = ""  # "ollama" or "openai"
SCRIPT_LLM_API_URL = ""   # e.g. "http://127.0.0.1:11434" or "https://api.example.com/v1"
SCRIPT_LLM_API_KEY = ""
SCRIPT_LLM_MODEL = ""     # e.g. "gemma3:4b"
...
def normalize_result(parsed, payload, source, model):
    ...
    if payload.get("mode") == "care_advice":
        ...
        summary = str(parsed.get("summary") or parsed.get("reply") or fallback["summary"]).strip()
        return {
            "ok": True,
            "source": source,
            "model": model,
            "riskLevel": risk_level,
            "statusLabel": str(parsed.get("statusLabel") or status_label(risk_level)).strip(),
    ...
    return {
        ...
        "summary": str(parsed.get("summary") or f"{payload.get('zoneName') or '此區域'}目前判定為{status_label(risk_level)}。").strip() if detail_mode else "",
        "situations": [str(item).strip() for item in parsed.get("situations", []) if str(item).strip()][:5] if detail_mode else [],
        "suggestions": [str(item).strip() for item in parsed.get("suggestions", []) if str(item).strip()][:5] if detail_mode else [],
    }
...
def read_args():
    ...
    parser.add_argument("--provider", choices=["ollama", "openai"], help="LLM provider mode")
...
def resolve_config(payload, args):
    config = {
        ...
        "provider": first_text(getattr(args, "provider", ""), SCRIPT_LLM_PROVIDER, os.environ.get("ZONE_ADVISOR_PROVIDER"), os.environ.get("LLM_PROVIDER")),
    }
