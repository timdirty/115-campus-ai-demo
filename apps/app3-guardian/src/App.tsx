...
import {evaluateCampusEvent, fetchBridgeHealth, fetchRobotEmotionEvents, fetchSensorPorts, fetchZoneInsight, fetchZoneSensors, getBridgeHost, pushGuardianSnapshot, pushRobotAssignment, resetBridgeDemoData, sendGuardianHardwareCommand, type RobotEmotionEvent, type ZoneInsightResponse} from './services/hardwareBridge';
import {AlertDetail, AlertRow, NodeRow} from './components/guardianUi';
...
import {GuardianControlPanel} from './components/GuardianControlPanel';
...
type DemoClosureStep = {label: string; detail: string; done: boolean; panel: Exclude<ActivePanel, null>};
type GuardianDemoRouteCard = {id: string; badge: string; title: string; line: string; next: string; icon: LucideIcon; action: () => void; disabled?: boolean};
type PrintableVisualScene = {
  id: string;
  title: string;
  zoneId: string;
  imageSrc: string;
  ...
  level: VisualPrivacyResult['level'];
  score: number;
  confidence: number;
  summary: string;
  evidence: string[];
  situations: string[];
  suggestions: string[];
  metrics: VisualPrivacyResult['metrics'];
  acoustic: Omit<AcousticSignal, 'id' | 'createdAt' | 'location'>;
};
type CreateVisualAlertOptions = {scene?: PrintableVisualScene; dispatchRobot?: boolean};
...
const PRINTABLE_SCENE_QUALITY: VisualPrivacyResult['quality'] = {
  level: 'good',
  label: '圖卡清晰',
  hints: ['示範圖卡適合列印或投影，系統只分析公共場域線索。'],
  metrics: {brightness: 64, contrast: 42, edgeDensity: 38, darkArea: 8, brightArea: 4},
};
const PRINTABLE_SCENE_SHEET_URL = 'demo-scenes/guardian-printable-scene-cards.html';
const PRINTABLE_VISUAL_SCENES: PrintableVisualScene[] = [
  {
    id: 'hallway-support',
    title: '穿堂下課關懷',
    zoneId: 'zone-hall',
    imageSrc: 'demo-scenes/guardian-scene-hallway-support.png',
    downloadName: 'guardian-scene-hallway-support.png',
    riskLevel: 'high',
    level: 'support',
    score: 74,
    confidence: 90,
    summary: '穿堂人流中出現匿名低互動停留情境，建議值勤老師用低壓方式靠近確認。',
    evidence: ['公共場域', '下課人流', '低互動停留', '不做人臉辨識'],
    situations: ['行政大樓 1F 下課人流快速通過。', '匿名學生停留在旁側，互動明顯偏低。'],
    suggestions: ['先派機器人到穿堂做柔和提醒。', '值勤老師只確認是否需要陪伴，不公開點名。'],
    metrics: {brightness: 66, motionEdges: 58, crowdTexture: 61, lowLightArea: 18},
    acoustic: {source: 'demo', level: 'elevated', volumeIndex: 74, volatility: 33, summary: '示範圖卡：穿堂下課人流較密集，與低互動場景一起進入融合判讀。'},
  },
  {
    id: 'library-pressure',
    title: '閱讀角考前壓力',
    zoneId: 'zone-library',
    imageSrc: 'demo-scenes/guardian-scene-library-pressure.png',
    downloadName: 'guardian-scene-library-pressure.png',
    riskLevel: 'medium',
    level: 'watch',
    score: 55,
    confidence: 84,
    summary: '圖書館閱讀角出現考前壓力與疲憊線索，建議導師提供任務拆解支持。',
    evidence: ['安靜區', '考前壓力', '低聲量環境', '不保存個人特徵'],
    situations: ['圖書館走廊接近考前自習時段。', '匿名學生呈現疲憊或壓力姿態，需要持續觀察。'],
    suggestions: ['提供考前任務拆解表。', '由導師或輔導老師在非公開場合短談。'],
    metrics: {brightness: 58, motionEdges: 34, crowdTexture: 38, lowLightArea: 22},
    acoustic: {source: 'demo', level: 'active', volumeIndex: 46, volatility: 16, summary: '示範圖卡：閱讀角聲量平穩，但壓力姿態需要納入關懷提醒。'},
  },
  {
    id: 'field-withdrawal',
    title: '操場邊緣退縮',
    zoneId: 'zone-field',
    imageSrc: 'demo-scenes/guardian-scene-field-withdrawal.png',
    downloadName: 'guardian-scene-field-withdrawal.png',
    riskLevel: 'high',
    level: 'support',
    score: 70,
    confidence: 88,
    summary: '操場邊緣出現匿名社交退縮情境，建議老師先確認安全與同儕互動狀態。',
    evidence: ['操場邊緣', '社交退縮', '老師觀察點', '只做場域判斷'],
    situations: ['操場活動區有人群，但匿名學生停在邊緣。', '場景適合示範老師如何低壓介入。'],
    suggestions: ['機器人先到操場邊提示老師注意。', '老師以陪伴式提問確認是否需要協助。'],
    metrics: {brightness: 62, motionEdges: 46, crowdTexture: 52, lowLightArea: 16},
    acoustic: {source: 'demo', level: 'active', volumeIndex: 61, volatility: 25, summary: '示範圖卡：操場活動聲量正常，但影像場景顯示需要關懷確認。'},
  },
];

function buildPrintableSceneResult(scene: PrintableVisualScene): VisualPrivacyResult {
  return {
    level: scene.level,
    label: scene.level === 'support' ? '需要老師關注' : '持續觀察',
    score: scene.score,
    summary: scene.summary,
    evidence: [...scene.evidence, 'GPT image 2 圖卡'],
    quality: PRINTABLE_SCENE_QUALITY,
    metrics: scene.metrics,
  };
}

function buildLocalPageUrl(path: string): string {
  if (typeof window === 'undefined') return path;
  return new URL(path, window.location.href).toString();
}

function getRobotDisplayUrl(): string {
...
function openStageWindow(url: string) {
  if (typeof window === 'undefined') return;
  window.open(buildLocalPageUrl(url), '_blank', 'noopener,noreferrer');
}
...
const PRINTABLE_SCENE_ASSESSMENT_LOCK_MS = 45000;
...
function AppContent() {
  ...
  const printableSceneLockRef = useRef<Record<string, number>>({});
  const baseZonesRef = useRef<SchoolZoneStatus[]>([]);
  const autoDispatchSeenRef = useRef<Record<string, RiskLevel>>({});
  const autoDispatchBootstrappedRef = useRef(false);
  ...
  const demoClosureSteps = useMemo(() => buildDemoClosureSteps(state, viewModel), [state, viewModel]);
  ...
  const refreshAllZoneAssessments = useCallback(async (zones: SchoolZoneStatus[], silent = false) => {
    ...
    await Promise.allSettled(zones.map(async (zone) => {
      const lockedUntil = printableSceneLockRef.current[zone.id] ?? 0;
      if (lockedUntil > Date.now()) return;
      if (lockedUntil > 0) delete printableSceneLockRef.current[zone.id];
  ...
  const createVisualAlert = useCallback((result: VisualPrivacyResult, options: CreateVisualAlertOptions = {}) => {
    if (result.level === 'calm' && !options.scene) {
      showToast('影像判讀為場域穩定，未建立提醒');
      return;
    }
    const scene = options.scene;
    const zone = scene
      ? viewModel.zones.find((item) => item.id === scene.zoneId) ?? selectedZone
      : viewModel.zones.find((item) => acousticLocation.includes(item.name) || item.location.includes(acousticLocation)) ?? selectedZone;
    const riskLevel: Exclude<RiskLevel, 'low'> = scene?.riskLevel ?? (result.level === 'support' ? 'high' : 'medium');
    const score = scene?.score ?? Math.max(result.score, riskLevel === 'high' ? 70 : 48);
    const statusLabel = riskLevel === 'high' ? '高風險' : '注意';
    const summary = scene?.summary ?? result.summary;
    const category = scene ? '影像圖卡辨識' : '隱私影像感知';
    const dispatchZone: SchoolZoneStatus = {...zone, riskLevel, riskScore: score, summary};

    if (scene) {
      printableSceneLockRef.current[zone.id] = Date.now() + PRINTABLE_SCENE_ASSESSMENT_LOCK_MS;
      const acousticSignal: Omit<AcousticSignal, 'id' | 'createdAt'> = {
        ...scene.acoustic,
        location: zone.name,
      };
      setAcousticLocation(zone.name);
      setCurrentAcoustic(acousticSignal);
      dispatch({type: 'RECORD_ACOUSTIC_SIGNAL', payload: acousticSignal});
    }
    ...
    dispatch({
      ...
      payload: {
        location: zone.location,
        type: scene ? `圖卡辨識：${scene.title}` : '影像場域提醒',
        description: `${summary} 判讀分數 ${score}/100。`,
        riskLevel,
        category,
        studentAlias: scene ? '匿名示範圖卡' : '匿名場域畫面',
        className: scene ? '國中進階 Demo' : undefined,
      },
    });
    recordZoneAssessment(dispatchZone, {
      ok: true,
      source: scene ? 'gpt-image-2-print-card' : 'local-privacy-camera',
      model: scene ? 'gpt-image-2' : null,
      riskLevel,
      statusLabel,
      confidence: scene?.confidence ?? Math.min(92, Math.max(62, result.score + 18)),
      summary,
      situations: scene?.situations ?? result.evidence.slice(0, 4),
      suggestions: scene?.suggestions ?? ['值勤老師先確認現場狀況，保持低壓關懷。', '不做身分辨識，不公開點名。'],
    });

    if (options.dispatchRobot) {
      dispatchRobotToZone(dispatchZone);
      setActivePanel('robot');
      showToast(`圖卡閉環：${zone.name}提醒已建立並派遣機器人`);
      return;
    }
    queueAutoDispatch(dispatchZone, scene ? `圖卡辨識：${scene.title}` : `影像場域：${result.label}`, riskLevel);
    setActivePanel('alerts');
    showToast(`${zone.name}影像提醒已加入預警`);
  }, [acousticLocation, dispatchRobotToZone, queueAutoDispatch, recordZoneAssessment, selectedZone, showToast, viewModel.zones]);
  ...
  useEffect(() => {
    ...
    if (!autoDispatchBootstrappedRef.current) {
      autoDispatchSeenRef.current = {};
      for (const zone of viewModel.zones) {
        if (zone.riskLevel !== 'low') {
          autoDispatchSeenRef.current[zone.id] = zone.riskLevel;
        }
      }
      autoDispatchBootstrappedRef.current = true;
      return;
    }
    for (const zone of viewModel.zones) {
      if (zone.riskLevel === 'low') {
  ...
  const runAutoDemo = useCallback(() => {
    ...
    const demoZone = viewModel.zones.find((zone) => zone.id === 'zone-hall') ?? viewModel.highestZone;
    const highZoneAssessment: ZoneInsightResponse = {
      ok: true,
      source: 'fallback',
      model: null,
      riskLevel: 'high',
      statusLabel: '高風險',
      confidence: 88,
      summary: '示範情境：午休後穿堂出現聲量波動與情緒壓力訊號，系統建議老師低壓確認。',
      situations: ['下課移動人流較密集。', '匿名心情文字與場域聲量同時升高。'],
      suggestions: ['先派機器人到場做柔和提醒。', '由導師或值勤老師確認，不公開點名。'],
    };
    const resolvedAssessment: ZoneInsightResponse = {
      ok: true,
      source: 'fallback',
      model: null,
      riskLevel: 'low',
      statusLabel: '安全',
      confidence: 92,
      summary: '示範閉環完成：老師確認後，場域回到一般巡查。',
      situations: ['機器人已到場提示，老師完成低壓確認。'],
      suggestions: ['保留紀錄並持續觀察，不標記個別學生。'],
    };
    autoDemoRunningRef.current = true;
    setAutoDemoRunning(true);
    clearLiveDemoRuntime();
    setSelectedAlert(null);
    setAutoDispatchQueue([]);
    setZoneAssessments({});
    autoDispatchSeenRef.current = {};
    autoDispatchBootstrappedRef.current = false;
    setSelectedZoneId(demoZone.id);
    setRobotDisplayZoneId(demoZone.id);
    dispatch({type: 'RESET_DEMO'});
    recordZoneAssessment(demoZone, highZoneAssessment);
    showToast('國中進階示範開始：5 段閉環展示…');
    autoDemoTimersRef.current.push(window.setTimeout(() => {
      dispatch({type: 'ADD_MOOD', payload: {mood: 'worried', label: '壓力升高', note: '示範：段考前焦慮與人際壓力同時出現'}});
      dispatch({
        type: 'CREATE_CONTEXT_ALERT',
        payload: {
          location: demoZone.location,
          type: '國中壓力事件',
          description: highZoneAssessment.summary,
          riskLevel: 'high',
          category: '多來源融合',
          studentAlias: '匿名壓力訊號',
          className: '八年級示範資料',
      ...
      setActivePanel('alerts');
      showToast('① 匿名心情 + 場域資料融合，建立高優先預警');
    }, 700));
    autoDemoTimersRef.current.push(window.setTimeout(() => {
      const elevated = {source: 'demo' as const, location: demoZone.name, level: 'elevated' as const, volumeIndex: 78, volatility: 39, summary: '示範：下課時間穿堂聲量偏高，與匿名壓力訊號一起進入 AI 融合判讀。'};
      setAcousticLocation(demoZone.name);
      setCurrentAcoustic(elevated);
      dispatch({type: 'RECORD_ACOUSTIC_SIGNAL', payload: elevated});
      setActivePanel('sensing');
      showToast('② 感知中心記錄聲量波動，補上第二個證據來源');
    }, 2300));
    autoDemoTimersRef.current.push(window.setTimeout(() => {
      dispatchRobotToZone(demoZone);
      setActivePanel('robot');
      showToast('③ 派遣機器人到穿堂，地圖與任務紀錄同步更新');
    }, 3900));
    autoDemoTimersRef.current.push(window.setTimeout(() => {
      const postId = `post-demo-${Date.now().toString(36)}`;
      dispatch({type: 'ADD_FOREST_POST', payload: {id: postId, content: '我可以先把壓力拆成一件一件小事，也可以找老師討論。', type: 'support'}});
      dispatch({type: 'SET_FOREST_POST_REPLY', payload: {id: postId, botReply: '很好，這是成熟的自我調節策略：先辨識壓力來源，再找可信任的大人一起處理。'}});
      dispatch({type: 'ADD_SUPPORT_MESSAGE', payload: {role: 'guardian', content: '示範回覆：先做安全確認，再用低壓方式陪學生整理壓力，不做診斷、不公開點名。'}});
      setActivePanel('care');
      showToast('④ 照護 Tab 留下學生支持與 AI 回覆證據');
    }, 5600));
    autoDemoTimersRef.current.push(window.setTimeout(() => {
      dispatch({
        type: 'COMPLETE_CARE_LOOP',
        payload: {
          area: demoZone.location,
          note: `${demoZone.location} 已完成預警、老師確認、機器人派遣、學生支持與結案紀錄。`,
        },
      });
      recordZoneAssessment(demoZone, resolvedAssessment);
      setActivePanel('robot');
      showToast('⑤ 閉環完成：預警結案、支援方案與硬體紀錄保留');
    }, 9300));
    autoDemoTimersRef.current.push(window.setTimeout(() => {
      autoDispatchSeenRef.current = {};
      autoDispatchBootstrappedRef.current = false;
  ...
  }, [viewModel.zones, viewModel.highestZone, showToast, recordZoneAssessment, dispatchRobotToZone, clearLiveDemoRuntime]);
  ...
  const resetStageDemo = () => {
    ...
    setRobotDisplayZoneId(null);
    robotLocationRef.current = null;
    setAutoDispatchQueue([]);
    setZoneAssessments({});
    printableSceneLockRef.current = {};
    autoDispatchSeenRef.current = {};
    autoDispatchBootstrappedRef.current = false;
    setCurrentAcoustic(defaultAcoustic);
    setAcousticLocation('穿堂');
    setSelectedMood('steady');
    setMessage('');
    setPostContent('');
    setManualEventText('');
    setManualEventBusy(false);
    setZoneInsightDialog(null);
    setMicError('');
    void resetBridgeDemoData();
    showToast('展示資料已重置');
  };

  const openRobotDisplayStage = useCallback(() => {
    setActivePanel('robot');
    openStageWindow(getRobotDisplayUrl());
  }, []);

  const guardianDemoRouteCards = useMemo<GuardianDemoRouteCard[]>(() => [
    {
      id: 'mood-care-loop',
      badge: '主線',
      title: '心情關懷閉環',
      line: '匿名心情、AI 低壓回覆、老師確認、結案留證。',
      next: autoDemoRunning ? '示範進行中' : '開始主線示範',
      icon: HeartHandshake,
      action: runAutoDemo,
    ...
    },
    {
      ...
      badge: '加分',
      title: '匿名圖卡閉環',
      line: '用匿名情境圖卡融合視覺、聲量與文字線索。',
      next: '開圖卡回答追問',
      icon: Camera,
      action: () => {
        setActivePanel('alerts');
        openStageWindow(PRINTABLE_SCENE_SHEET_URL);
        showToast('匿名情境圖卡已開啟，可展示「提醒老師確認」而非診斷');
      },
    },
  ], [autoDemoRunning, createProactiveAlert, runAutoDemo, showToast]);
    ...
    <div className="guardian-shell min-h-screen overflow-x-hidden bg-[linear-gradient(160deg,#f5f9fc_0%,#eef3f8_60%,#e8f0f7_100%)] text-slate-950">
      ...
      <header className="sticky top-0 z-50 border-b border-slate-200/60 bg-white/95 shadow-[0_1px_12px_rgba(15,23,42,0.06)] backdrop-blur-xl">
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
          ...
          <StageStatusPill bridgeOnline={bridgeOnline} sensorCount={zoneSensors.filter((s) => s.connected).length} />

          <div className="flex items-center gap-2">
            <button
              onClick={runAutoDemo}
              disabled={autoDemoRunning}
              className="flex min-h-11 items-center justify-center gap-2 rounded-xl bg-teal-600 px-4 text-sm font-black text-white shadow-sm shadow-teal-200 transition hover:bg-teal-700 active:scale-[0.98] disabled:bg-slate-300 disabled:text-slate-500 disabled:shadow-none"
              title="執行完整示範流程（訊號→預警→派遣→照護→結案）"
            >
              <Radar className={`h-4 w-4 ${autoDemoRunning ? 'animate-spin' : ''}`} />
              <span>{autoDemoRunning ? '示範中' : '開始示範'}</span>
            </button>
            <button
              onClick={resetStageDemo}
              className="hidden min-h-11 items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-xs font-black text-slate-700 shadow-sm transition hover:border-teal-200 hover:text-teal-700 sm:flex"
            >
      ...
      <main className="mx-auto grid max-w-7xl gap-4 px-4 py-4 pb-24 sm:px-6 lg:pb-8 lg:grid-cols-[minmax(0,1fr)_22rem]">
        ...
        <aside className="hidden lg:flex lg:flex-col gap-4 lg:sticky lg:top-21 lg:max-h-[calc(100vh-6rem)] lg:overflow-y-auto">
          ...
          <RobotDisplaySync
            ...
            focusMode={activePanel === 'robot'}
          />
        </aside>
      </main>

      {/* Mobile bottom 4-tab nav */}
      ...
      <DetailDrawer
        ...
        onCreateProactiveAlert={createProactiveAlert}
        onCreateVisualAlert={createVisualAlert}
        onDemoSound={() => {
...
function StagePilotStrip({
  ...
  totalCount: number;
  onRunAutoDemo: () => void;
  onOpenPrintableScenes: () => void;
  onOpenRobotDisplay: () => void;
  onReset: () => void;
}) {
  const readyItems = [
    {label: '閉環', value: `${doneCount}/${totalCount}`, good: doneCount === totalCount},
    {label: '橋接', value: bridgeOnline ? '在線' : '離線備援', good: bridgeOnline},
    {label: '圖卡', value: `${PRINTABLE_VISUAL_SCENES.length} 張`, good: PRINTABLE_VISUAL_SCENES.length >= 3},
    {label: '感測', value: sensorCount > 0 ? `${sensorCount} 組` : '可模擬', good: sensorCount > 0},
  ];
  ...
  return (
    <Surface className="lg:col-span-2 p-3 sm:p-4">
      <div className="flex flex-col gap-3 lg:flex-row lg:items-center lg:justify-between">
        <div className="min-w-0">
          <p className="text-xs font-black tracking-wide text-teal-600">學生上台模式</p>
          <h2 className="mt-1 text-xl font-black tracking-tight text-slate-950 sm:text-2xl">一排按鈕完成示範</h2>
        </div>
        <div className="grid grid-cols-2 gap-2 sm:grid-cols-4 lg:min-w-[26rem]">
          {readyItems.map((item) => (
            <div key={item.label} className={`rounded-xl border px-3 py-2 ${item.good ? 'border-teal-200 bg-teal-50 text-teal-800' : 'border-amber-200 bg-amber-50 text-amber-800'}`}>
              <p className="text-[10px] font-black text-current/70">{item.label}</p>
      ...
      <div className="mt-3 grid gap-2 sm:grid-cols-4">
        <button
          ...
          disabled={autoDemoRunning}
          className="flex min-h-12 items-center justify-center gap-2 rounded-xl bg-teal-600 px-4 text-sm font-black text-white shadow-sm transition hover:bg-teal-700 active:scale-[0.98] disabled:bg-slate-300 disabled:text-slate-500"
        >
          <Radar className={`h-4 w-4 ${autoDemoRunning ? 'animate-spin' : ''}`} />
          {autoDemoRunning ? '示範中' : '開始示範'}
        </button>
        <button type="button" onClick={onOpenPrintableScenes} className="flex min-h-12 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-black text-slate-700 transition hover:border-teal-200 hover:text-teal-700">
          <Printer className="h-4 w-4" />
          開圖卡
        </button>
        <button type="button" onClick={onOpenRobotDisplay} className="flex min-h-12 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-white px-4 text-sm font-black text-slate-700 transition hover:border-teal-200 hover:text-teal-700">
          <Bot className="h-4 w-4" />
          機器人幕
        </button>
        <button type="button" onClick={onReset} className="flex min-h-12 items-center justify-center gap-2 rounded-xl border border-slate-200 bg-slate-50 px-4 text-sm font-black text-slate-700 transition hover:border-rose-200 hover:bg-rose-50 hover:text-rose-700">
          <RefreshCw className="h-4 w-4" />
          重置舞台
        </button>
      </div>
...
function GuardianDemoRouteCards({routes}: {routes: GuardianDemoRouteCard[]}) {
  return (
    <Surface className="lg:col-span-2 p-3 sm:p-4">
      <div className="mb-3 flex flex-col gap-1 sm:flex-row sm:items-end sm:justify-between">
        <div>
          <p className="text-xs font-black tracking-wide text-teal-600">展示路線卡</p>
          <h2 className="text-xl font-black tracking-tight text-slate-950 sm:text-2xl">主線先跑，支線回答追問</h2>
        </div>
        <p className="text-xs font-bold text-slate-500">所有語氣都用提醒、陪伴、老師確認</p>
      </div>
      <div className="grid gap-3 md:grid-cols-3">
        {routes.map((route) => {
          const Icon = route.icon;
          const isPrimary = route.badge === '主線';
          return (
            <button
              key={route.id}
              type="button"
              ...
              className={`min-h-36 rounded-2xl border p-4 text-left shadow-sm transition active:scale-[0.99] disabled:cursor-wait disabled:opacity-70 ${
                isPrimary
                  ? 'border-teal-600 bg-teal-600 text-white shadow-lg shadow-teal-200'
                  : 'border-slate-200 bg-slate-50 text-slate-800 hover:border-teal-200 hover:bg-white'
              }`}
            >
              <div className="flex items-center gap-3">
                <span className={`grid h-11 w-11 shrink-0 place-items-center rounded-xl ${isPrimary ? 'bg-white/20 text-white' : 'bg-teal-50 text-teal-700'}`}>
                  <Icon className="h-6 w-6" />
                </span>
                <span className="min-w-0">
                  <span className={`block text-[10px] font-black tracking-[0.18em] ${isPrimary ? 'text-white/70' : 'text-teal-600'}`}>{route.badge}</span>
                  <span className={`block text-base font-black leading-tight ${isPrimary ? 'text-white' : 'text-slate-950'}`}>{route.title}</span>
                </span>
              </div>
              <p className={`mt-3 text-sm font-bold leading-6 ${isPrimary ? 'text-white/80' : 'text-slate-600'}`}>{route.line}</p>
              <p className={`mt-3 text-xs font-black ${isPrimary ? 'text-white' : 'text-teal-700'}`}>{route.next} →</p>
            </button>
          );
        })}
      </div>
...
function CommandCenterScreen({
  ...
  demoClosureSteps: DemoClosureStep[];
...
function DemoClosureRail({steps, onOpenPanel, onRunAutoDemo, autoDemoRunning}: {steps: DemoClosureStep[]; onOpenPanel: (panel: ActivePanel) => void; onRunAutoDemo: () => void; autoDemoRunning: boolean}) {
  const doneCount = steps.filter((step) => step.done).length;
  return (
    <div data-tour="demo-loop">
    <Surface className="p-3 sm:p-4">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <p className="text-xs font-black tracking-wide text-teal-600">上台流程</p>
          <h3 className="mt-1 text-lg font-black text-slate-950">五段閉環 {doneCount}/{steps.length}</h3>
        </div>
        <button
          type="button"
          onClick={onRunAutoDemo}
          disabled={autoDemoRunning}
          className="flex min-h-11 items-center justify-center gap-2 rounded-xl bg-slate-950 px-4 text-sm font-black text-white shadow-sm transition hover:bg-slate-800 active:scale-[0.98] disabled:bg-slate-300 disabled:text-slate-500"
        >
          <Radar className={`h-4 w-4 ${autoDemoRunning ? 'animate-spin' : ''}`} />
          {autoDemoRunning ? '示範中' : '跑完整示範'}
        </button>
      </div>
      <div className="mt-3 h-2 overflow-hidden rounded-full bg-slate-100">
        <div className="h-full rounded-full bg-teal-500 transition-all" style={{width: `${(doneCount / steps.length) * 100}%`}} />
      </div>
      <div className="mt-3 grid grid-cols-5 gap-1.5">
        {steps.map((step, index) => (
          <button
            ...
            className={`flex min-h-16 flex-col items-start gap-1.5 rounded-xl border p-2 text-left transition active:scale-[0.99] sm:min-h-20 sm:flex-row sm:gap-2 sm:p-3 ${
              step.done
          ...
          >
            <span className={`flex h-6 w-6 shrink-0 items-center justify-center rounded-lg text-[10px] font-black ${
              step.done ? 'bg-teal-600 text-white' : 'bg-white text-slate-500 ring-1 ring-slate-200'
            }`}>
              {index + 1}
            </span>
            <span className="min-w-0">
              <span className="block text-[11px] font-black leading-tight sm:text-sm">{step.label}</span>
              <span className="mt-1 hidden text-[10px] font-bold leading-4 opacity-75 sm:block">{step.detail}</span>
            </span>
          </button>
        ))}
      </div>
    ...
    <div className="mb-3 overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
      ...
      <div className="grid gap-2 md:grid-cols-3">
        {zones.map((zone) => {
          ...
          const sourceLabel = !assessment ? '待判讀' : assessment.source === 'fallback' ? '備援規則' : 'AI 判讀';
        ...
        <div className="p-5">
          ...
          <div className="mt-4 grid gap-3 sm:grid-cols-[1fr_1.4fr]">
            <div className={`rounded-2xl border p-4 ${tone.panel}`}>
              ...
              <p className="mt-2 text-xs font-bold text-slate-500">燈號由 AI 依感測器數值判斷；沒有回應時使用本機備援規則。</p>
            </div>
          ...
          <div className="mt-4 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 sm:flex-row sm:items-center sm:justify-between">
            <div>
              <p className="text-xs font-black text-slate-500">AI 設定</p>
...
function DetailDrawer(props: {
  ...
  onCreateProactiveAlert: () => void;
  onCreateVisualAlert: (result: VisualPrivacyResult, options?: CreateVisualAlertOptions) => void;
  onDemoSound: () => void;
            ...
            <div className="min-h-0 flex-1 overflow-y-auto py-4 pb-safe">
              ...
              {panel === 'robot' && (
                <div className="space-y-4">
                  <RobotReadinessCard state={props.state} robotFeedback={props.robotFeedback} />
                  <MissionTimeline state={props.state} robotFeedback={props.robotFeedback} />
  ...
  return (
    <div className="grid gap-4">
      ...
      <button
        ...
        className="flex min-h-11 items-center justify-center gap-2 rounded-xl border border-teal-200 bg-teal-50 px-4 text-sm font-black text-teal-700 shadow-sm transition hover:bg-teal-100 active:scale-[0.98]"
      >
...
function SensingPanel({
  ...
  const runPrintableScene = (scene: PrintableVisualScene) => {
    const result = buildPrintableSceneResult(scene);
    setVisualResult(result);
    setVisualError('');
    setVisualAnalyzedAt(`圖卡 ${new Intl.DateTimeFormat('zh-TW', {hour: '2-digit', minute: '2-digit', second: '2-digit', hour12: false}).format(new Date())}`);
    onCreateVisualAlert(result, {scene, dispatchRobot: true});
  };
...
function StageStatusPill({bridgeOnline, sensorCount}: {bridgeOnline: boolean; sensorCount: number}) {
  return (
    <div className="hidden items-center gap-2 rounded-full border border-teal-200 bg-teal-50 px-3 py-1.5 text-xs font-black text-teal-700 md:flex">
      <span className={`h-2 w-2 rounded-full ${bridgeOnline ? 'bg-emerald-500' : 'bg-amber-400'}`} />
      {bridgeOnline ? (sensorCount > 0 ? `硬體在線・${sensorCount} 感測` : '硬體在線') : '舞台模式・可離線展示'}
    </div>
  );
}

function StageToolsMenu({
  onRestartTour,
  onOpenSensorSetup,
  onOpenPrintableScenes,
  onOpenRobotDisplay,
  ...
  const items = [
    {label: '導覽', icon: Radar, onClick: onRestartTour},
    {label: '感測器', icon: Settings, onClick: onOpenSensorSetup, attention: sensorAttention},
    {label: '列印圖卡', icon: Printer, onClick: onOpenPrintableScenes},
    {label: '機器人幕', icon: ExternalLink, onClick: onOpenRobotDisplay},
    {label: '匯出', icon: Download, onClick: onExport},
    {label: '匯入', icon: Upload, onClick: onImport},
    {label: '重置', icon: RefreshCw, onClick: onReset},
  ];
  return (
    <details className="group relative">
      <summary className="flex min-h-11 cursor-pointer list-none items-center gap-2 rounded-xl border border-slate-200 bg-white px-3 text-xs font-black text-slate-700 shadow-sm transition hover:border-teal-200 hover:text-teal-700 [&::-webkit-details-marker]:hidden">
        <Settings className="h-4 w-4" />
        <span className="hidden sm:inline">工具</span>
      </summary>
      <div className="absolute right-0 top-13 z-60 grid w-44 gap-1 rounded-2xl border border-slate-200 bg-white p-2 shadow-xl shadow-slate-950/10">
        {items.map((item) => (
          <button
            key={item.label}
            type="button"
            ...
            className="flex min-h-10 items-center justify-between gap-2 rounded-xl px-3 text-left text-xs font-black text-slate-700 transition hover:bg-teal-50 hover:text-teal-700"
          >
            <span className="flex items-center gap-2">
              <item.icon className="h-4 w-4" />
              {item.label}
            </span>
            {item.attention && <span className="h-2 w-2 rounded-full bg-amber-400" />}
          </button>
        ))}
      </div>
    </details>
