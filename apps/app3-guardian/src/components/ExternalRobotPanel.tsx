...
import {getBridgeBaseUrl} from '../services/hardwareBridge';
...
async function fetchStatus(hw: RobotHW): Promise<HWStatus | null> {
  try {
    const res = await fetch(`${getBridgeBaseUrl()}/api/${hw}/status`, {signal: AbortSignal.timeout(2000)});
...
async function sendCmd(hw: RobotHW, command: string): Promise<boolean> {
  try {
    const res = await fetch(`${getBridgeBaseUrl()}/api/${hw}/command`, {
      method: 'POST',
