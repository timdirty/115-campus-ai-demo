...
export type GuardianAction =
  ...
  | {type: 'COMPLETE_CARE_LOOP'; payload: {area: string; note: string}}
...
export function guardianReducer(state: GuardianState, action: GuardianAction): GuardianState {
  ...
  switch (action.type) {
    ...
    case 'COMPLETE_CARE_LOOP': {
      const area = action.payload.area.trim() || '全校';
      const matchesArea = (alert: GuardianAlert) =>
        area === '全校' ||
        alert.location.includes(area) ||
        area.includes(alert.location) ||
        alert.description.includes(area) ||
        action.payload.note.includes(alert.location);

      return {
        ...state,
        alerts: state.alerts.map((alert) =>
          alert.status !== 'resolved' && matchesArea(alert)
            ? {
                ...alert,
                status: 'resolved',
                checklist: alert.checklist.map((item) => ({...item, completed: true})),
              }
            : alert,
        ),
        interventions: [
        ...
        supportMessages: [
          ...state.supportMessages,
          {
            ...
            content: `${area}的關懷流程已完成：先辨識匿名訊號，再由老師低壓確認，最後留下結案紀錄。`,
            createdAt: now,
          },
        ].slice(-30),
