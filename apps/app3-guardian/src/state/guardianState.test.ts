...
async function run() {
  ...
  const loopClosed = guardianReducer(robotCompleted, {
    type: 'COMPLETE_CARE_LOOP',
    payload: {area: '穿堂', note: '穿堂已完成老師確認、機器人派遣、學生支持與追蹤紀錄。'},
  });
  assert.equal(loopClosed.alerts.find((item) => item.location.includes('穿堂'))?.status, 'resolved');
  assert.ok(loopClosed.alerts.find((item) => item.location.includes('穿堂'))?.checklist.every((item) => item.completed));
  assert.equal(loopClosed.interventions[0].status, 'completed');
  assert.match(loopClosed.supportMessages.at(-1)?.content ?? '', /關懷流程已完成/);
