...
const ENV_BRIDGE_URL =
  ((import.meta as unknown as {env?: Record<string, string>}).env?.VITE_ARDUINO_BRIDGE_URL || '').trim();

export function getBridgeBaseUrl(): string {
  if (ENV_BRIDGE_URL) return ENV_BRIDGE_URL.replace(/\/+$/, '');
  if (typeof window !== 'undefined') {
    const host = window.location.hostname || 'localhost';
    return `http://${host}:3203`;
  }
...
export function getBridgeHost(): string {
  try {
    return new URL(getBridgeBaseUrl()).host;
  } catch {
...
async function doGuardianPost(command: string, source: string) {
  ...
  try {
    ...
    const payload = await response.json().catch(() => ({}));
    const payloadOk = typeof payload.ok === 'boolean' ? payload.ok : response.ok;
    return {
      ok: payloadOk,
