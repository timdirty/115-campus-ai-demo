...
/* ════════════════════════════════════════════════════════════════
   自我介紹腳本（國中進階 Demo，總長約 18 秒）
   ════════════════════════════════════════════════════════════════ */
const INTRO_SCRIPT = [
  { text: '大家好，我是 AI 校園心靈守護者的機器人前端。', emotion: 'focused' },
  { text: '我接收主控台的匿名風險訊號與派遣任務，不辨識身分、不做診斷。', emotion: 'calm' },
  { text: '當系統發現需要關懷的場域，我會顯示目的地與任務狀態。', emotion: 'focused' },
  { text: '我協助老師低壓確認現場，讓關懷流程可以被看見、被完成。', emotion: 'happy' }
];
...
function getBridgeHttpUrl(input) {
  if (!input) input = 'localhost:3203';
  if (/^wss?:\/\//.test(input) || /^https?:\/\//.test(input)) {
    try {
      const u = new URL(input);
  ...
  const clean = input.replace(/^wss?:\/\//, '').replace(/^https?:\/\//, '').replace(/\/display$/, '');
...
export default function App() {
  ...
  const missionActive = mission?.active === true || mission?.moving === true;
  const missionStatus = mission?.stage || (missionActive ? '任務執行中' : '現場待命');
  ...
  const [bridgeInput, setBridgeInput] = React.useState(() => {
    const fromUrl = new URLSearchParams(window.location.search).get('bridge')?.trim();
    if (fromUrl) {
      localStorage.setItem(BRIDGE_KEY, fromUrl);
      return fromUrl;
    }
    const stored = localStorage.getItem(BRIDGE_KEY);
    if (stored) return stored;
    return 'localhost:3203';
  });
  ...
  useEffect(() => {
    const t1 = setTimeout(() => {
      _setBubble('我是 AI 校園心靈守護者的機器人前端。\n點我聽任務介紹，或等待主控台派遣。', 8000);
    }, 900);
  ...
  const publishRobotEmotionEvent = useCallback((key, origin = 'scan') => {
    if (!['anxious', 'sad', 'stressed'].includes(key)) return;
    const info = EMOTIONS[key];
    if (!info) return;
    const riskLevel = key === 'stressed' ? 'high' : 'medium';
    const activeMission = mission;
    const payload = {
      emotion: key,
      emotionLabel: info.zh,
      riskLevel,
      zoneId: activeMission?.zoneId || 'zone-field',
      zoneName: activeMission?.zoneName || '操場',
      location: activeMission?.location || activeMission?.zoneName || '操場',
      description: `robot_app3 ${origin} 偵測到${info.zh}狀態，建議主控台建立低壓關懷確認。`,
      source: 'robot_app3',
    };
  ...
  }, [introPlaying, tts, voiceEnabled, stt]);
  ...
  /* ─────── 開始任務掃描 ─────── */
  const startScan = () => {
    if (scanning || introPlaying) return;
    setBubble(null);
    tts.stop(); stt.stop();
    setLeftOpen(false); setRightOpen(false); setBottomOpen(false);
    audio.unlock();
    ...
    setTimeout(() => {
      ...
      publishRobotEmotionEvent(next, 'emotion-scan');
    }, 2400);
  };
  ...
  bcHandlerRef.current = (data) => {
    ...
    if (data.type === 'robot_assignment') {
      const nextMission = {
        zoneId: data.zoneId || 'zone-field',
        zoneName: data.zoneName || '操場',
        location: data.location || data.zoneName || '操場',
        riskLevel: data.riskLevel || 'low',
        statusLabel: data.statusLabel || '校園守護',
        stage: data.stage || '現場待命',
        missionId: data.missionId || null,
        active: data.active === true,
        moving: data.moving === true,
        fromZoneName: data.fromZoneName || null,
        updatedAt: data.updatedAt || new Date().toISOString(),
      };
      setMission(nextMission);
      const missionEmotion = nextMission.moving
        ? 'focused'
        : nextMission.riskLevel === 'high'
          ? 'stressed'
          : nextMission.riskLevel === 'medium'
            ? 'anxious'
            : 'calm';
      if (EMOTIONS[missionEmotion]) {
        setEmotion(missionEmotion);
        const t = new Date();
        const tStr = `${String(t.getHours()).padStart(2, '0')}:${String(t.getMinutes()).padStart(2, '0')}`;
        setHistory(h => [{ e: missionEmotion, t: tStr }, ...h.slice(0, 4)]);
      }
      _setBubble(`收到主控任務：${nextMission.zoneName}\n${nextMission.stage} · ${nextMission.statusLabel}`, 8000);
    }
  ...
  const handleRobotTap = (ev) => {
    ...
    const tapResponses = [
      '我正在待命，可以接收主控台的派遣任務。',
      '下方可以做任務掃描，若有守護訊號會回報主控台。',
      '我協助老師低壓確認現場，不公開個人身分。',
      '任務同步正常，請看左上角或下方儀表板。',
      '收到，持續守護校園場域。',
      '需要我示範任務流程時，可以按開始任務掃描。'
    ];
  ...
  return (
    <div
      ...
      style={{
        background: `
          radial-gradient(ellipse 60% 40% at 50% 0%, ${e.glow} 0%, transparent 60%),
          radial-gradient(ellipse 80% 50% at 50% 100%, ${e.light} 0%, transparent 55%),
    ...
    >
        ...
        {bridgeEditing && (
          <div style={{
            ...
            <div style={{display:'flex',gap:8}}>
              <button
                onClick={() => {
                  const nextBridge = bridgeDraft.trim() || 'localhost:3203';
                  localStorage.setItem(BRIDGE_KEY, nextBridge);
                  setBridgeDraft(nextBridge);
                  setBridgeInput(nextBridge);
      ...
      <div className="absolute left-1/2 -translate-x-1/2 pointer-events-none"
        style={{
          ...
          background: `radial-gradient(circle, ${e.glow} 0%, transparent 65%)`,
      ...
      <div className="absolute top-5 left-5 right-5 flex items-start justify-between z-30 pointer-events-none">
        <button
          ...
          className="pointer-events-auto flex items-center gap-2.5 pl-2 pr-4 py-2 bg-white/85 backdrop-blur-xl rounded-full shadow-lg border border-white/60 transition-all hover:scale-[1.02] active:scale-95"
        >
          <div
      ...
      {mission && (
        <div className="absolute left-4 right-4 bottom-44 z-20 pointer-events-none sm:bottom-auto sm:left-5 sm:right-auto sm:top-20 sm:max-w-[360px]">
          <div className="rounded-3xl border border-white/70 bg-white/88 px-4 py-3 shadow-xl backdrop-blur-xl">
            <div className="mb-1 flex items-center justify-between gap-3">
              <span className="text-[9px] font-bold tracking-[0.22em] text-stone-500">APP3 MISSION</span>
              <span
                className="rounded-full px-2 py-0.5 text-[9px] font-bold tracking-widest"
                style={{
                  background: missionActive ? `${e.color}22` : '#f1f5f9',
                  color: missionActive ? e.color : '#64748b',
                }}
              >
            ...
            <div className="flex items-start justify-between gap-3">
              <div className="min-w-0">
                <div className="truncate font-display text-xl font-bold tracking-tight text-stone-900">{mission.zoneName}</div>
                <div className="mt-1 line-clamp-2 text-xs font-medium leading-5 text-stone-600">
                  {mission.fromZoneName && mission.moving ? `${mission.fromZoneName} → ${mission.zoneName}` : mission.location}
                </div>
              </div>
              <div className="shrink-0 text-right">
                <div className="text-xs font-bold" style={{ color: e.color }}>{missionStatus}</div>
                <div className="mt-1 text-[10px] font-mono-tight text-stone-500">{mission.statusLabel}</div>
              </div>
            </div>
          </div>
        </div>
              ...
              <div className="rounded-2xl p-4 mb-6 bg-white border border-stone-200">
                ...
                <button
                  onClick={() => {
                    audio.unlock();
                    audio.play('speak');
                    tts.speak('你好，我是 AI 校園心靈守護者的機器人前端。');
                  }}
      ...
      <Drawer position="right" open={rightOpen} onClose={() => setRightOpen(false)}>
        <div className="px-6 pt-6 pb-8">
          ...
          <h2 className="font-display text-3xl font-bold text-stone-900 tracking-tight mb-1">任務測試</h2>
          <p className="text-xs text-stone-500 mb-6">Stage test · 測試任務狀態與語音</p>
          ...
          <div className="text-[10px] tracking-[0.2em] text-stone-500 font-bold uppercase mb-3">任務狀態測試</div>
