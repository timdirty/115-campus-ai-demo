...
def fallback_params(text, person_count):
    ...
    if any(w in text for w in ["疲倦", "想睡", "低落", "趴", "放空"]):
        emotion = "sad"
        stress, stability, focus = 58, 48, 38
        advice = "班級精神稍低，建議安排短暫伸展或提問互動。"
    elif any(w in text for w in ["分散", "不穩定", "焦躁", "緊張", "混亂"]):
        emotion = "anxious"
...
def normalize_params(parsed, raw_text, person_count):
    ...
    emotion = parsed.get("emotion", fallback["emotion"])
...
def build_frontend_payload(params):
...
# LLM analysis
# =====================
def analyze(frame, should_broadcast=True, verbose=True):
...
"""
    ...
    payload = build_frontend_payload(params)
...
def run_once(camera_index=0):
    frame = capture_frame(camera_index)
    payload = analyze(frame, should_broadcast=False, verbose=False)
