...
import {
  ...
  Loader2, AlertCircle, MapPin, Bot
} from 'lucide-react';
...
/* ════════════════════════════════════════════════════════════════
   自我介紹腳本（國中進階展示，總長約 20 秒）
   ════════════════════════════════════════════════════════════════ */
const INTRO_SCRIPT = [
  { text: '大家好，我是 AI 校園心靈守護者的機器人前端。', emotion: 'happy' },
  { text: '我接收主控台的匿名風險訊號與派遣任務，不辨識身分、不做診斷。', emotion: 'calm' },
  { text: '當系統發現需要關懷的場域，我會顯示目的地與任務狀態。', emotion: 'focused' },
  { text: '我的任務是提醒老師低壓確認，讓同學安全被看見。', emotion: 'happy' }
];
...
const bridgeFromQuery = () => {
  if (typeof window === 'undefined') return '';
  return new URLSearchParams(window.location.search).get('bridge')?.trim() || '';
};
...
/* ════════════════════════════════════════════════════════════════
   ...
   ════════════════════════════════════════════════════════════════ */
const useSpeech = () => {
  const [supported] = useState(() => typeof window !== 'undefined' && Boolean(window.speechSynthesis));
  ...
  useEffect(() => {
    ...
    return () => {
      try { window.speechSynthesis.onvoiceschanged = null; } catch { /* ignore unsupported cleanup */ }
      clearTimeout(t1); clearTimeout(t2); clearTimeout(t3);
    };
  }, [applyVoice, supported]);
  ...
  const stop = useCallback(() => {
    if (!supported) return;
    try { window.speechSynthesis.cancel(); } catch { /* ignore unsupported cleanup */ }
    setSpeaking(false);
  }, [supported]);
...
const useListen = () => {
  ...
  const stop = useCallback(() => {
    try { recRef.current?.stop(); } catch { /* ignore unsupported cleanup */ }
    recRef.current = null;
    setListening(false);
  }, []);
...
export default function App() {
  ...
  const [bridgeInput, setBridgeInput] = React.useState(() => {
    ...
    if (fromQuery) {
      localStorage.setItem(BRIDGE_KEY, fromQuery);
      return fromQuery;
    }
  ...
  /* 純設定泡泡 */
  const _setBubble = useCallback((text, duration = 6000) => {
  ...
  }, [_setBubble, introPlaying, isMoving, tts, voiceEnabled, stt]);
  ...
  const startScan = async () => {
    ...
    try {
      ...
      setScanning(false);
      audio.play('error');
      showBubble('任務掃描暫時失敗，請改由主控台派遣或稍後再試。', 6000);
    }
  };
  ...
  // 每次渲染後更新 handler，確保 WebSocket 使用最新閉包
  useEffect(() => {
    bcHandlerRef.current = (data) => {
    // =========================
        ...
        {bridgeEditing && (
          <div style={{
            ...
            <div style={{display:'flex',gap:8}}>
              <button
                onClick={() => {
                  const nextBridge = bridgeDraft.trim() || defaultBridgeHost();
          ...
          {displayAssignment && (
            ...
            >
              ...
              <div className="min-w-0 text-left">
                <div className="text-[9px] tracking-[0.2em] text-stone-500 font-bold uppercase leading-none">APP3 Mission</div>
