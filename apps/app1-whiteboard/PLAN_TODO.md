...
國小課堂白板 AI 助教：拍下白板、整理老師講解、保存筆記、產生複習問答，並用 UNO R4 WiFi 示範老師可控的半自動板擦機器人動作。
...
- 已把作品對外說法收斂成「白板辨識 + 老師確認 + 區塊式板擦」，避免過度宣稱全自動定位。
...
- 實機上確認伺服角度與板擦機構左區 / 右區的物理位置；這只屬於工程模式。
...
6. 學生能清楚說明「目前是半自動區塊控制，下一階段才是視覺定位閉環」。
7. Arduino 未連線時保留任務紀錄，不中斷展示。
