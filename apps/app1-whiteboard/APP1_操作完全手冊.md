...
> Firmware 位置：`firmware/app1-whiteboard-drive/main.cpp`，L293D M3/M4 馬達。
...
> ⚠️ **Gemini 的 Fallback 區塊座標（A/B）是預設百分比，不代表你現場白板的實際位置。校準是必要步驟，不要跳過。**
...
> 「沒有 Gemini Key 時，系統走本機展示模式——白板整理、A/B 區塊建議、課堂筆記、學習單、AI 小老師，全部流程都能完整跑完，不會出錯誤訊息。正式部署給學校用時，由後端接 Gemini API，分析品質就會升級為真正的 AI 分析。」
...
- [ ] 決定上台分工（固定 3 位：說明的人、操作的人、收尾與回答的人）
