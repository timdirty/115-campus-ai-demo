...
1. 接上實體板擦機器人
2. 在網址後加 `?engineerTools=1`
3. 只在實機偏移時，校正左區、右區與待命位置
4. 學生展示與一般老師操作不需要碰這些參數

> ⚠️ **主流程只使用左區 / 右區兩個大區。工程校正是接實體機器人時的維護工具，不是學生展示步驟。**
...
> 「沒有 Gemini Key 時，系統走本機展示模式——白板整理、左區 / 右區建議、課堂筆記、學習單、AI 小老師，全部流程都能完整跑完，不會出錯誤訊息。正式部署給學校用時，由後端接 Gemini API，分析品質就會升級為真正的 AI 分析。」
...
A：第一階段先解決最重要的事：白板內容保存、AI 協助判斷、老師確認、機器人依左區或右區執行。固定攝影機與即時定位是第二階段，會放在工程模式處理，不放進學生主流程。

**Q：這個成本多少？**  
A：硬體約 1,500 元（Arduino + 馬達 + 材料），軟體完全自製。比市售白板清潔設備便宜很多。

**Q：下一步計畫是什麼？**  
A：加入固定攝影機做白板座標校正，讓機器人定位更精準，實現閉環自動控制；但展示主流程仍保持左區 / 右區兩大區，避免操作變複雜。
