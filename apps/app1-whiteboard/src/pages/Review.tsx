...
export default function Review({ onNavigate }: { onNavigate: (tab: string) => void }) {
  ...
  const handleGenerate = async () => {
    ...
    try {
      ...
      setErrorText(error instanceof Error ? error.message : '生成失敗，請重新整理後再試一次。');
      setViewState('setup');
    }
  };
      ...
      <AnimatePresence mode="wait">
        {viewState === 'setup' && (
          <motion.div key="setup" initial={{opacity:0, scale:0.95}} animate={{opacity:1, scale:1}} exit={{opacity:0, scale:0.95}}>
            ...
            <div className="grid grid-cols-1 md:grid-cols-12 gap-5 md:gap-8 lg:gap-10" data-tour="learning-sheet">
