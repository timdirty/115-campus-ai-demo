import { Search, Eye, Folder, ChevronRight, LayoutGrid, List, Trash2, X, Brain, PenTool, Sparkles, Download, Edit3, Loader2, Save, Check } from 'lucide-react';
      ...
      <section className="mb-10 sm:mb-14">
        ...
        {searchQuery === '' && (
          <StageRecordPanel
            ...
            onOpen={() => latestNote && setSelectedNote(latestNote)}
            onChat={() => { saveDemoProgress({library: true}); onNavigate('chat'); }}
            onReview={() => { saveDemoProgress({library: true}); onNavigate('review'); }}
          />
        )}
      </section>
              ...
              <div className="w-full md:w-[55%] flex flex-col bg-surface flex-1 min-h-0">
                ...
                <div className="p-5 md:p-6 bg-surface-container-lowest shrink-0 border-t border-outline-variant/10 flex flex-col gap-2">
                  ...
                  {isEditing ? (
                    ...
                    <>
                      <button onClick={() => { saveDemoProgress({library: true}); setSelectedNote(null); onNavigate('chat'); }} className="flex-1 bg-surface-container hover:bg-primary-container hover:text-primary text-on-surface font-extrabold py-3.5 px-4 rounded-xl flex items-center justify-center gap-2 transition-colors shadow-sm text-sm">
                        <Brain className="w-4 h-4" /> 問 AI 小老師
                      </button>
                      <button onClick={() => { saveDemoProgress({library: true}); setSelectedNote(null); onNavigate('review'); }} className="flex-1 bg-primary text-on-primary font-extrabold py-3.5 px-4 rounded-xl flex items-center justify-center gap-2 transition-all hover:bg-primary-dim hover:shadow-md active:scale-95 text-sm">
...
function StageRecordPanel({
  ...
  return (
    <motion.div variants={itemVariants} className="mb-6 rounded-lg border border-primary/15 bg-surface-container-lowest p-4 sm:p-5 shadow-sm">
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[1fr_18rem] lg:items-center">
        <button
          type="button"
          onClick={onOpen}
          disabled={!note}
          className="flex min-h-32 w-full items-center gap-4 rounded-md border border-outline-variant/15 bg-surface p-3 text-left transition-all hover:border-primary/30 hover:bg-primary-container/20 active:scale-[0.99] disabled:opacity-60"
        >
          <div className="h-24 w-36 shrink-0 overflow-hidden rounded-md bg-surface-container sm:h-28 sm:w-44">
            {note?.img ? <img className="h-full w-full object-cover" src={note.img} alt={note.title} /> : <div className="h-full w-full bg-surface-container" />}
          </div>
          <div className="min-w-0">
            <div className="mb-2 inline-flex items-center gap-2 rounded-full bg-primary-container px-3 py-1 text-xs font-black text-primary">
              <Check className="h-3.5 w-3.5" />
              最新紀錄已保存
            </div>
            <h3 className="line-clamp-2 text-xl font-extrabold leading-tight">{note?.title ?? '尚無課堂紀錄'}</h3>
            <p className="mt-1 line-clamp-2 text-sm font-semibold text-on-surface-variant">{note?.desc ?? '先回白板頁按一鍵示範。'}</p>
          </div>
        </button>

        <div className="rounded-md border border-primary/10 bg-primary-container/25 p-4">
          <p className="text-xs font-black text-primary">學生上台下一步</p>
          <p className="mt-1 text-sm font-bold text-on-surface-variant">確認有保存，再選一個展示收尾。</p>
          <div className="mt-4 grid grid-cols-1 gap-2">
            <button data-demo-primary="library" onClick={onChat} className="min-h-11 rounded-md bg-surface px-4 text-sm font-extrabold text-primary transition hover:bg-primary hover:text-on-primary">
              問小老師
            </button>
            <button onClick={onReview} className="min-h-11 rounded-md bg-primary px-4 text-sm font-extrabold text-on-primary transition hover:bg-primary-dim">
              生成學習單
            </button>
          </div>
          <p className="mt-3 text-xs font-semibold text-on-surface-variant">目前 {noteCount} 筆；展示重跑會覆寫同一筆，不會洗版。</p>
        </div>
      </div>
    </motion.div>
  );
