...
export default function Chat({ onNavigate }: { onNavigate: (tab: string) => void }) {
  ...
  const submitMessage = async (text: string) => {
    ...
    try {
      ...
      const noteIds = (matchedNote ? [matchedNote.id] : notes.slice(0, 1).map((note) => note.id));
  ...
  return (
    ...
    >
      ...
      <aside className="hidden lg:flex lg:w-[32%] flex-col gap-4 sm:gap-6 hide-scrollbar overflow-y-auto lg:pb-8 lg:pr-2 lg:shrink-0">
        <motion.div variants={messageVariants} className="bg-surface-container-low rounded-3xl p-4 sm:p-6 flex flex-col gap-3 sm:gap-4 border border-outline-variant/10 shadow-sm relative overflow-hidden group">
          <div className="flex items-center justify-between relative z-10">
            ...
            <button type="button" onClick={() => onNavigate('library')} aria-label="切換至課堂紀錄" className="w-10 h-10 rounded-full bg-surface shadow-sm cursor-pointer hover:scale-105 active:scale-95 flex items-center justify-center transition-transform">
      ...
      {/* Main Chat Interface */}
      <motion.section variants={messageVariants} className="flex-1 flex flex-col bg-surface-container-lowest rounded-3xl md:rounded-[2.5rem] editorial-shadow overflow-hidden border border-outline-variant/10 relative w-full max-w-full" data-tour="ai-teacher">
        {/* Header Options */}
        ...
        <div className="shrink-0 border-b border-outline-variant/10 bg-surface-container-lowest px-4 py-3 sm:px-6">
          <p className="mb-2 text-xs font-black text-primary">學生可直接按</p>
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-3">
            {QUICK_PROMPTS.map((prompt) => (
              <button
                key={prompt}
                type="button"
                data-demo-primary={prompt === QUICK_PROMPTS[0] ? 'chat' : undefined}
                onClick={() => submitMessage(QUICK_PROMPT_MESSAGES[prompt])}
                disabled={isTyping}
                className="min-h-11 rounded-md bg-surface px-3 text-sm font-extrabold text-on-surface transition hover:bg-primary hover:text-on-primary active:scale-95 disabled:opacity-50"
              >
                {prompt}
              </button>
            ))}
        ...
        <div className="p-3 sm:p-4 lg:p-5 bg-surface shrink-0 border-t border-outline-variant/10 z-20">
          <form onSubmit={(e) => { e.preventDefault(); submitMessage(inputValue); }} className="relative flex items-end group shadow-inner bg-surface-container-highest rounded-[1.25rem] sm:rounded-[1.5rem] border border-transparent focus-within:border-primary/40 focus-within:bg-surface focus-within:shadow-[0_4px_20px_rgba(80,107,79,0.08)] transition-all p-1.5 sm:p-2">
            ...
            <div className="absolute right-1.5 bottom-1.5 sm:right-2 sm:bottom-2 flex">
              <button type="submit" disabled={!inputValue.trim() || isTyping} className={`w-10 h-10 rounded-xl sm:rounded-2xl flex items-center justify-center transition-all duration-300 ${!inputValue.trim() || isTyping ? 'bg-surface-container text-on-surface-variant/40' : 'bg-primary text-on-primary shadow-md hover:scale-105 active:scale-95'}`}>
