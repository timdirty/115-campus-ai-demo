      ...
      <div className="mt-4 rounded-2xl border border-outline-variant/20 bg-surface-container p-4">
        ...
        {showCalibration && (
          ...
          <div className="mt-3 grid grid-cols-1 gap-2 text-[11px] font-bold text-on-surface-variant md:grid-cols-3">
            <div className="rounded-xl bg-surface px-3 py-2">範圍模式：{calibrationMode === 'auto' ? '自動偵測' : calibrationMode === 'manual' ? '手動微調' : '預設值'}</div>
            <div className="rounded-xl bg-surface px-3 py-2">清楚程度：{detectionConfidence}%</div>
