...
import {getBridgeBase, getStoredBridgeHost, setBridgeHost} from '../services/apiClient';
import {resetWhiteboardDemoState} from '../services/notesStore';
...
function friendlyCheckMessage(name: string, ok: boolean) {
  if (name === 'storage' || name === 'static-page') return ok ? '資料可保存' : '資料保存需要檢查';
  if (name === 'static-build') return ok ? '展示包已準備' : '展示包尚未產生';
  if (name === 'hardware-link') return ok ? '展示模式可操作' : '機器人連動需要檢查';
  return ok ? '已就緒' : '需要檢查';
}
...
export default function SystemSettingsPanel({onClose}: SystemSettingsPanelProps) {
  ...
  const handleDemoReset = () => {
    ...
    fetch(`${getBridgeBase()}/api/ops/reset`, {method: 'POST'}).catch(() => {});
  ...
  return (
    <div className="bg-surface w-full max-w-2xl rounded-2xl sm:rounded-3xl p-5 sm:p-8 shadow-2xl border border-white/50 relative max-h-[88dvh] overflow-y-auto">
      ...
      <div className="pr-12">
        ...
        <p className="text-sm text-on-surface-variant mt-2">管理展示狀態、資料備份與匯入。</p>
      </div>
      ...
      <div className="mt-6 grid grid-cols-1 sm:grid-cols-2 gap-3">
        ...
        <StatusBlock
          icon={KeyRound}
          title="AI 助教"
          value={ready?.geminiConfigured ? '已連線' : '展示模式'}
          ok={true}
          loading={loadingReady}
        />
        <StatusBlock
      ...
      {ready && (
        <div className={`mt-4 flex items-start gap-3 rounded-xl border p-4 ${
          ...
          <div>
            ...
            <p className="text-xs mt-1 break-words">{ready.checks.map((check) => friendlyCheckMessage(check.name, check.ok)).join('；')}</p>
          </div>
        </div>
      )}
      ...
      {/* 跨裝置連線設定 */}
      <div className="mt-6 rounded-2xl bg-surface p-4 border border-outline-variant/10 space-y-3">
      ...
      {/* Demo 重置 */}
      <div className="mt-4 rounded-2xl bg-error-container/30 p-4 border border-error/20 space-y-2">
