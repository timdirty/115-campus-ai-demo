...
const BOARD_TEMPLATES: ChatTemplate[] = [
  {
    ...
    answer: '我們把白板分成左區、右區兩個大區。AI 分析完畢後，會判斷每一區的內容是否已經完整傳達，標記「可清空」或「保留」。老師在教師看板確認後，機器人才會動作——AI 只是建議，老師有最終決策權。',
  },
  {
  ...
  },
  {
    ...
    answer: '目前系統把白板分成左區、右區兩個大區。老師只要決定左邊或右邊要保留、可清空，不需要處理複雜座標。',
  },
  {
  ...
  },
  {
    ...
    answer: '學生展示不需要調校正參數。白板預設只分左區和右區；接上實體機器人時，老師會在離線階段用彈簧 + 軌道機構物理校準位置，學生主流程不會碰到這些細節。',
  },
  {
    keywords: ['多次', '重複', '再次', '連續', '批次'],
    answer: '可以連續送出左右兩區的任務。教師看板有「連續擦除」模式，依序執行左區、右區，或只擦標記為可清空的區域。',
  },
];
...
const ROBOT_TEMPLATES: ChatTemplate[] = [
  {
    ...
    answer: '板擦機器人會依照老師指定的左區或右區移動。收到任務後，它會先到指定位置，再執行擦除或等待。',
  },
  {
  ...
  },
  {
    ...
    answer: '機器人控制頁面可以送出常用任務，例如擦除左區、擦除右區、全板擦除、暫停等待、成功動畫。學生展示時只要按畫面上的中文按鈕即可。',
  },
  {
    keywords: ['伺服', 'servo', '角度', '校準', '調整'],
    answer: '我們改用彈簧 + 軌道機構固定板擦臂，不需要伺服角度調整。實機安裝時直接物理校準，學生展示走主流程，所有參數已預先 tune 好。',
  },
  {
  ...
  },
  {
    ...
    answer: '我們設計「AI 建議 + 老師確認」的 HITL 流程：AI 用 Gemini Vision 看白板給建議，老師最後確認哪些區塊該擦。機器人會用標準測試樣本自我驗證殘留，沒達標自動再擦（最多 3 次）。這是國小教室的安全設計，不是 AI 做不到全自動。',
  },
  {
