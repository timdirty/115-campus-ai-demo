            ...
            <div className={fullScreen ? "flex-1 flex flex-col overflow-hidden" : "overflow-y-auto flex-1 relative"}>
              {!title && fullScreen && (
                <button
                  onClick={onClose}
                  aria-label="關閉全螢幕面板"
                  className="absolute top-4 right-4 z-50 p-2 w-9 h-9 rounded-full bg-black/50 text-white backdrop-blur-md transition-colors flex items-center justify-center active:scale-90"
                >
                  <X size={20} />
                </button>
              )}
