...
import {buildRobotDisplayCue, robotDisplayCueSignature, type RobotDisplayCue} from '../services/robotDisplayCue';
...
export const RobotDisplaySync = memo(function RobotDisplaySync() {
  ...
  const [currentCue, setCurrentCue] = useState<RobotDisplayCue>(() => buildRobotDisplayCue(state));
  const statusPollRef = useRef<ReturnType<typeof setInterval> | null>(null);
  const lastCueSignatureRef = useRef('');
  ...
  const sendCue = useCallback(async (cue: RobotDisplayCue) => {
    const ctrl = new AbortController();
    const t = setTimeout(() => ctrl.abort(), 3000);
    try {
      const res = await fetch(`${BRIDGE_URL}/api/display/cue`, {
        method: 'POST',
      ...
      if (res.ok) {
        const json = await res.json() as {clients: number};
        setConnected(json.clients > 0);
        setRobotEmotion(cue.emotion as EmotionKey);
        setCurrentCue(cue);
        const now = new Date();
  ...
  useEffect(() => {
    if (!autoSync) return;
    const cue = buildRobotDisplayCue(state);
    const signature = robotDisplayCueSignature(cue);
    setCurrentCue(cue);
    setRobotEmotion(cue.emotion as EmotionKey);
    if (signature === lastCueSignatureRef.current) return;
    lastCueSignatureRef.current = signature;
    void sendCue(cue);
  }, [autoSync, sendCue, state]);
      ...
      >
        ...
        <div className="flex-1 min-w-0">
          ...
          <p className="text-sm font-bold text-on-surface flex items-center gap-1.5">
            <span>{currentCue.mission}</span>
            {connected
              ? <span className="text-[10px] text-emerald-600 font-bold">同步中</span>
              : <span className="text-[10px] text-slate-400 font-bold">可展示</span>}
          </p>
          <p className="mt-0.5 truncate text-[10px] font-bold text-on-surface-variant/70">{currentCue.status}</p>
        </div>
          ...
          <div className="flex items-center gap-2 rounded-xl bg-surface-container-lowest p-2.5 ring-1 ring-outline-variant/20">
            ...
            <div className="flex-1 min-w-0">
              <p className="text-xs font-bold text-on-surface-variant">
                {connected ? '機器人表情會同步更新' : '可先用主畫面完成展示'}
              </p>
              {lastSync && <p className="text-[10px] font-mono text-on-surface-variant/60">最後推送 {lastSync}</p>}
            </div>
          </div>

          <div className="rounded-xl bg-primary/10 p-3 ring-1 ring-primary/15">
            <p className="text-[10px] font-black tracking-widest text-primary uppercase">機器人會說</p>
            <p className="mt-1 text-sm font-bold leading-5 text-on-surface">{currentCue.message}</p>
          </div>
              ...
              {EMOTION_KEYS.map((key) => {
                ...
                return (
                  <button
                    ...
                    type="button"
                    onClick={() => {
                      const cue: RobotDisplayCue = {
                        emotion: key,
                        mission: EMOTIONS[key].label,
                        status: '手動表情測試',
                        message: `我是${EMOTIONS[key].label}表情，機器人顯示連線正常。`,
                        source: 'system',
                        command: 'DISPLAY_MANUAL',
                      };
                      lastCueSignatureRef.current = robotDisplayCueSignature(cue);
                      void sendCue(cue);
                      setAutoSync(false);
                    }}
