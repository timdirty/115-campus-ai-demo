    ...
    <BottomSheet isOpen={isOpen} onClose={onClose} title="手動遙控中心">
      <div className="overflow-y-auto flex flex-col gap-5 p-5 pb-6">
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-outline-variant/20 bg-surface-container-low/60 p-3">
          ...
          <span className={`rounded-full px-3 py-1 text-[10px] font-black ${bridgeOnline ? 'bg-primary/10 text-primary' : 'bg-emerald-50 text-emerald-700'}`}>
            {bridgeOnline ? '機器人同步' : '展示操作'}
          </span>
        </div>
...
export function RemoteControlLauncher() {
  ...
  return (
    <AnimatePresence>
      <RemoteControlFab key="remote-fab" onOpen={() => setOpen(true)} />
      <RemoteControlPanel key="remote-panel" isOpen={open} onClose={() => setOpen(false)} />
    </AnimatePresence>
  );
}
