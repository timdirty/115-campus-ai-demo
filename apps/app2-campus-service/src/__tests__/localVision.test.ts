...
import {VISION_DEMO_SCRIPTS, analyzeCampusFrame, analyzeCampusPixels, createScriptedVisionResult} from '../services/localVision';
...
function run() {
  ...
  assert.ok(lowQuality.evidence.some((item) => item.includes('光線偏暗')));

  const scripted = createScriptedVisionResult(VISION_DEMO_SCRIPTS[0], 'camera-unavailable', {
    llmOk: false,
    backupUsed: true,
    rawScene: 'crowd',
    error: 'camera-unavailable',
  });
  assert.equal(scripted.isReliable, true);
  assert.equal(scripted.dispatchRecommended, true);
  assert.equal(scripted.aiSource, 'scripted');
  assert.equal(scripted.teacherDebug?.backupUsed, true);
  assert.ok(scripted.studentLine?.includes('機器人'));

  const demoCards = [
    ['01-crowd-hallway.png', 'crowd'],
    ['02-safety-patrol.png', 'safety'],
    ['03-cleaning-needed.png', 'cleaning'],
    ['04-delivery-service.png', 'delivery'],
    ['05-normal-patrol.png', 'patrol'],
  ] as const;
  for (const [file, scene] of demoCards) {
    const card = PNG.sync.read(fs.readFileSync(`public/demo-assets/vision-cards/${file}`));
    const cardResult = analyzeCampusPixels(card.width, card.height, new Uint8ClampedArray(card.data));
    assert.equal(cardResult.scene, scene, `${file} should classify as ${scene}`);
    assert.equal(cardResult.isReliable, true, `${file} should be reliable enough for camera-card demo`);
    assert.equal(cardResult.dispatchRecommended, true, `${file} should allow dispatch in the closed loop`);
  }
