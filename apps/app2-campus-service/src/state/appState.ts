    ...
    robotCommandLogs: [
      {
        ...
        mode: 'demo',
        status: 'demo-only',
        note: '展示流程已準備好，可以建立任務、派遣機器人並保留服務紀錄。',
      },
    ],
    logs: [
      { id: 'log-1', time: '04:12', message: '展示：校園服務流程已啟動', type: 'info' },
      { id: 'log-2', time: '04:12', message: '影像：生活場景辨識已準備', type: 'info' },
...
export function appReducer(state: AppState, action: AppAction): AppState {
  ...
  switch (action.type) {
    ...
    case 'ADD_DISPATCH_TASK': {
      const sourceTitle = action.payload.title ?? (action.payload.taskType === 'broadcast' ? '群眾疏導' : '自動巡邏');
      const robotId = action.payload.robotId ?? (action.payload.taskType === 'broadcast' ? '3號' : '1號');
      const status: RobotStatus = action.payload.robotStatus ?? (action.payload.taskType === 'broadcast' ? '疏導' : '巡邏');
      const detail = action.payload.detail ?? (action.payload.taskType === 'broadcast' ? '校園廣播與人流疏導' : '巡邏熱區並回傳影像');
      const phase = action.payload.phase ?? (action.payload.taskType === 'broadcast' ? '廣播中' : '巡邏中');
      ...
      return withRobotCommandLog(
        ...
        now,
        {
          command: action.payload.command ?? (action.payload.taskType === 'broadcast' ? 'BROADCAST_START' : 'PATROL_START'),
    ...
    case 'MARK_HARDWARE_COMMAND':
      return {
        ...
        logs: addLog(
          state,
            action.payload.ok ? `機器人：${action.payload.message}` : `展示紀錄：任務已保留`,
