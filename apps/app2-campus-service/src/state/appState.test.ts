...
import { buildRobotDisplayCue } from '../services/robotDisplayCue';
...
async function run() {
  ...
  const deliveryCue = buildRobotDisplayCue(ordered);
  assert.equal(deliveryCue.mission, '配送服務');
  assert.equal(deliveryCue.emotion, 'happy');
  assert.match(deliveryCue.message, /前往|送到/);
  ...
  const teachingCue = buildRobotDisplayCue(attendanceScanned);
  assert.equal(teachingCue.mission, '教學協助');
  assert.match(teachingCue.message, /點名/);
  ...
  const dispatchCue = buildRobotDisplayCue(dispatched);
  assert.equal(dispatchCue.mission, '人流疏導');
  assert.equal(dispatchCue.emotion, 'excited');
  ...
  const emergencyCue = buildRobotDisplayCue(locked);
  assert.equal(emergencyCue.mission, '全校安全');
  assert.match(emergencyCue.message, /安全提醒/);
