...
async function checkArduinoReady(): Promise<boolean> {
  const controller = new AbortController();
  const timeoutId = setTimeout(() => controller.abort(), 900);
  try {
    const response = await fetch(`${BRIDGE_URL}/api/health`, {signal: controller.signal});
    const payload = await response.json().catch(() => ({})) as {arduinoConnected?: boolean};
...
export async function sendHardwareCommand(command: string, source: string): Promise<HardwareBridgeResult> {
  const arduinoReady = await checkArduinoReady();
  if (!arduinoReady) {
    return {ok: false, statusCode: 0, message: '展示模式：硬體尚未連線，任務已保留'};
  }
