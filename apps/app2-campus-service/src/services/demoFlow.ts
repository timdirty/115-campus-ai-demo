...
export function getDemoSteps(state: AppState): DemoStep[] {
  ...
  return [
    ...
    {id: 'report', label: '報表收尾', detail: '服務紀錄可以說明', done: hasReportReady},
  ];
}

export function getDemoHealth(state: AppState): DemoHealthStatus[] {
  return [
    {label: '任務資料', value: `${state.tasks.length} 任務 / ${state.logs.length} 日誌`, ok: state.tasks.length > 0},
    {label: '派遣紀錄', value: `${state.robotCommandLogs.length} 筆紀錄`, ok: state.robotCommandLogs.length > 0},
    {label: '機隊狀態', value: `${state.robots.filter((robot) => robot.isRunning).length} 台執行中`, ok: state.robots.length >= 4},
    {label: '展示狀態', value: state.hardwareMode === 'serial-ready' ? '已同步' : '展示就緒', ok: true},
  ];
}
