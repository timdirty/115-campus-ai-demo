...
export type ClassroomScanSignal = {type: 'question' | 'distracted'; description: string};
export type ClassroomScanResult = {count: number; rate: number; confidence: number; summary: string; signals: ClassroomScanSignal[]};
...
- 若不確定或看不到學生，count 填 0，confidence 填低於 40，signals 填 []`;
  ...
  const signals: ClassroomScanSignal[] = rawSignals
