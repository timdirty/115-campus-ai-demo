...
export const RobotDisplaySync = memo(function RobotDisplaySync() {
  ...
  }, [state.tasks, state.campusStatus.isEmergency, autoSync]);

  /* ── 教學告警暫時覆寫 ── */
  useEffect(() => {
    if (!autoSync) return;
    if (state.campusStatus.isEmergency) return;
    const classroomBehaviorAlerts = state.teachingSignals
      .filter((signal) => signal.type === 'alert' && isClassroomBehaviorMessage(signal.message))
      .slice(0, 3);
    const hasClassroomBehaviorAlert = classroomBehaviorAlerts.length > 0;
    if (!hasClassroomBehaviorAlert) {
      alertReplySeqRef.current += 1;
      if (alertResetRef.current) {
        clearTimeout(alertResetRef.current);
        alertResetRef.current = null;
    ...
    const alertMessage = classroomBehaviorAlerts.map((signal) => signal.message).join('；') || '上課有學生分心，請提醒專注。';
