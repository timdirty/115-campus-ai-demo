...
/**
 * Analyze a camera frame — tries Gemini via bridge (2s timeout), falls back to rotating demo cycle.
 */
export async function analyzeCampusImageWithGemini(
  imageDataUrl: string,
  cancelSignal?: AbortSignal,
): Promise<CampusVisionResult & {aiSource?: 'gemini' | 'pixel'}> {
  try {
    ...
    return {
      ...
      evidence: [`Gemini 視覺辨識`, `信心度 ${data.confidence ?? '?'}%`],
      aiSource: data.source === 'gemini' ? 'gemini' : 'pixel',
    };
  } catch {
