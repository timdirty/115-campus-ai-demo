    ...
    case 'SET_DEMO_MODE': {
      if (action.payload.enabled) {
        return {
          ...
          lastUpdated: now,
          logs: addLog(createDemoAppState(), '系統：展示模式已開啟，示範資料已載入', 'info', now),
        };
      }
      return clearDemoData(
        {
          ...state,
          logs: addLog(state, '系統：展示模式已關閉，示範資料已清空', 'info', now),
        },
        now,
      );
    }
    ...
    case 'RESET_DEMO':
      return state.settings.demoMode
        ? {
            ...
            settings: {...state.settings, demoMode: true},
            lastUpdated: now,
            logs: addLog(createDemoAppState(), '系統：展示資料已重置', 'info', now),
          }
