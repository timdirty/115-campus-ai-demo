...
import { MessageCircle, AlertCircle, Send, Activity, Focus, ArrowUpRight, Mic, ImagePlus, Camera } from 'lucide-react';
...
function getClassroomAtmosphere(analysis: ClassroomAnalysisResult | null) {
  if (!analysis || analysis.source === 'local') {
    return {
      label: analysis ? 'LLM 分析中' : '等待環場分析',
      tone: 'bg-white/40',
      glow: 'shadow-[0_0_10px_rgba(255,255,255,0.35)]',
      detail: analysis ? '正在等待 LLM 回傳專注度與氛圍描述' : '尚未取得氛圍資料',
    };
  }
...
function getLearningAlerts(analysis: ClassroomAnalysisResult | null) {
  if (!analysis || analysis.source === 'local') {
    return [
      {label: analysis ? 'LLM 分析中' : '等待掃描', detail: analysis ? '已取得影像，正在等待 LLM 產生專注度、學習警示與氛圍描述。' : '尚未取得教室影像，請用即時氛圍或上傳氛圍照片開始分析。'},
      {label: '資料來源', detail: '點名人數由 YOLO 負責；專注度、學習警示與氛圍描述由 LLM 負責。'},
    ];
  }
...
function getVisionAlerts(analysis: ClassroomAnalysisResult): VisionAlert[] {
  if (Array.isArray(analysis.visualAlerts) && analysis.visualAlerts.length) {
    return analysis.visualAlerts.map((item) => ({
      label: item.label,
  ...
  if (/手機|滑手機|phone|mobile/.test(text)) {
    alerts.push({label: '疑似滑手機', message: '畫面疑似有學生使用手機或電子裝置，請老師確認。'});
  }
  if (/睡|睡覺|趴|趴睡|低頭很久|tired|sleep/.test(text)) {
    alerts.push({label: '疑似睡覺', message: '畫面疑似有學生睡覺或長時間低頭，請老師確認。'});
  }
  if (/舉手|raise|hand/.test(text)) {
    alerts.push({label: '疑似舉手', message: '畫面偵測到疑似舉手或求助訊號，請老師留意。'});
  }
  if (analysis.emotion === 'distracted' || analysis.motion === 'restless') {
    alerts.push({label: '分心躁動', message: '班級畫面出現分心或躁動趨勢，建議暫停確認學習狀態。'});
  }
  return alerts.filter((item, index, list) => list.findIndex((candidate) => candidate.message === item.message) === index).slice(0, 3);
}
...
export function TeachView({ showToast, navigateTo }: { showToast: (m: string) => void, navigateTo: (id: string, props?: any) => void }) {
  ...
  const hasLlmClassroomAnalysis = classroomAnalysis?.source === 'gemini' || classroomAnalysis?.source === 'ollama';
  const classroomSourceLabel = classroomAnalysis?.source === 'gemini' ? 'Gemini' : classroomAnalysis?.source === 'ollama' ? 'Ollama/Gemma' : 'LLM 分析中';
  const atmosphere = getClassroomAtmosphere(classroomAnalysis);
  const learningAlerts = getLearningAlerts(classroomAnalysis);
  const hasClassroomScan = hasLlmClassroomAnalysis;
  ...
  useEffect(() => {
    if (!classroomAnalysis || classroomAnalysis.source === 'local') {
      setFocusScore(0);
    ...
    if (detections.length) {
	  ...
	  const publishFastClassroomAnalysis = (
	    ...
	    setClassroomAnalysis({
	      ...quick,
	      summary: `${quick.summary} Gemini 分析完成後會自動覆蓋。`,
    ...
    async function loop() {
      while (!ctrl.signal.aborted) {
	        ...
	        if (capture) {
	          ...
	          const analysisPromise = analyzeClassroomFrame(capture, previousFrameRef.current, ctrl.signal);
  ...
  const handleRollCall = () => {
    ...
    if (!camReady) {
      ...
      showToast('請先允許攝影機，系統會用影像框選並每 3 秒讓 LLM 檢查氛圍');
      return;
    }
    setModal('video');
    setTrackingActive(true);
    resetVideoTracking();
    showToast('即時氛圍分析啟動：影像框選，每 3 秒由 LLM 複核');
  };
  ...
  const handleTestVideo = (file: File | undefined) => {
    ...
    setClassroomPreviewImage(null);
    resetVideoTracking();
    showToast('模擬實時畫面已載入：影片會當成鏡頭來源，每 3 秒分析一次');
        ...
        <div className="flex-1 min-w-0 relative z-10">
           ...
           {attendanceScan ? (
             ...
             <div className="mt-1">
                ...
                <div className="mt-1 text-[10px] text-primary/70 animate-pulse flex items-center gap-1"><span className="w-1.5 h-1.5 rounded-full bg-primary inline-block"></span> 攝影機或照片皆可測試</div>
             </div>
           )}
        </div>
      ...
      <div role="button" tabIndex={0} aria-label="開啟班級專注度分析報表" onKeyDown={(e) => e.key === 'Enter' && setModal('chart')} className="bg-surface-container-lowest rounded-2xl p-5 relative overflow-hidden shadow-md border border-outline-variant/30 cursor-pointer hover:bg-surface-container transition-all group active:scale-[0.98]" onClick={() => setModal('chart')}>
          <div className="flex justify-between items-start mb-4 relative z-10">
            <div>
              ...
              <p className="mt-1 text-[10px] font-bold tracking-widest text-on-surface-variant">
                {classroomAnalyzing || classroomAnalysis?.source === 'local' ? 'LLM 學習氛圍分析中' : hasClassroomScan ? `${classroomSourceLabel} 氛圍分析` : '等待即時複核或氛圍照片'}
              </p>
            </div>
          ...
          <div className="h-14 flex items-end gap-1.5 px-1 relative z-10">
            {waveData.map((h, i) => (
              <motion.div
                key={i}
                animate={{ height: `${hasClassroomScan ? Math.max(8, h) : 8}%` }}
                transition={{ type: "spring", bounce: 0.2, duration: 0.8 }}
                className={`flex-1 rounded-t-md mx-[1px] ${hasClassroomScan && i === waveData.length - 1 ? 'bg-primary shadow-[0_0_15px_rgba(var(--color-primary),0.6)]' : 'bg-primary/50'}`}
                style={{ opacity: hasClassroomScan ? 0.4 + (h/100)*0.6 : 0.18 }}
              />
            ))}
          </div>
        ...
        <div className="flex items-center justify-between gap-3 mb-3">
          <div>
            ...
            <p className="text-[11px] text-on-surface-variant font-medium mt-0.5">
              {classroomAnalyzing || classroomAnalysis?.source === 'local' ? '正在交由 LLM 分析專注度、學習警示與氛圍描述，完成後會自動更新。' : classroomAnalysis ? atmosphere.detail : '完成點名辨識後會自動更新專注度、氛圍描述與警示'}
            </p>
          </div>
        </div>
        ...
        <div className="mb-3 grid grid-cols-1 gap-2 md:grid-cols-[0.95fr_1.35fr]">
          ...
          <div className="rounded-xl border border-outline-variant/25 bg-surface-container px-4 py-3">
            ...
            <p className="mt-2 text-sm font-bold leading-relaxed text-on-surface">
              {classroomAnalyzing || classroomAnalysis?.source === 'local' ? 'LLM 正在分析課堂氛圍，完成後會顯示自然語意描述。' : classroomAnalysis?.summary ?? '完成點名辨識後，會接續描述課堂氛圍與可疑學習狀態。'}
            </p>
          </div>
        </div>
        ...
        <div className="w-full h-full bg-black relative flex flex-col justify-center overflow-hidden">
          ...
          {!videoFeedReady && (
            <div className="absolute inset-0 flex flex-col items-center justify-center gap-4 pointer-events-none"
              ...
              {testVideoUrl ? (
                <p className="text-white/50 text-sm font-mono animate-pulse">載入模擬實時畫面中…</p>
              ) : camError ? (
                <>
            ...
            <div className="flex justify-between items-end gap-3">
              <div className="flex-1 min-w-0">
                <h3 className="text-white font-headline font-bold text-xl drop-shadow-md truncate">
                  {testVideoUrl ? `101 教室 · 模擬實時畫面` : camZone || '101 教室 · 即時場域'}
                </h3>
                <p className="text-white/60 text-sm font-medium mt-1 font-mono">
                  {classroomAnalyzing ? 'LLM + YOLO 分析中…' : (videoFeedReady ? (testVideoUrl ? '模擬鏡頭監控中' : 'Gemini Vision 監控中') : testVideoUrl ? '模擬畫面載入中…' : '攝影機啟動中…')}
                </p>
                ...
                {(classroomAnalysis || camScene) && !camAnalyzing && (
                  <div className="mt-2 flex items-center gap-2">
                    ...
                    <span className="text-[10px] text-white/50 font-mono">
                      {classroomAnalysis ? (classroomAnalysis.source === 'gemini' ? 'Gemini' : classroomAnalysis.source === 'ollama' ? 'Ollama Gemma' : 'LLM 分析中') : (camSource === 'gemini' ? 'Gemini' : camSource === 'ollama' ? 'Ollama Gemma' : '本地分析')}
                    </span>
                  </div>
                )}
                {trackingActive && (
              ...
              <div className={`shrink-0 px-3 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest flex items-center gap-2 shadow-lg ${videoFeedReady ? 'bg-error/80 backdrop-blur text-white' : 'bg-black/60 text-white/40'}`}>
                ...
                {videoFeedReady ? (testVideoUrl ? '模擬實時' : '實況錄製') : '待機'}
              </div>
            </div>
          </div>
        </div>
