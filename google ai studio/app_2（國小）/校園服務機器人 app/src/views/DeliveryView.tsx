...
const INITIAL_PRODUCTS = [
  { id: 1, name: '特級厚片土司', price: 28, desc: '現烤厚片土司，外酥內軟，香氣撲鼻。', img: 'https://lh3.googleusercontent.com/aida-public/AB6AXuCyJiSdhIHLuHc8TyD-U7oEtD5XYkmt899rllhPyiGrKxL0Pkx88vtHnk4zpEiZ2F4RjSy3_ynW8vDpIeXn0dOUaf9FjRXz2j3oq5gs-iHEVj9c_hyJUNy5442mlGtJJ3d5JN2FaTOfo_Ud-2BpVR7kKcd3r_yHYQDY3aFq8y4Tu4hN74HB_Ez2gPdqdxwFJbOrdwzoKTgx7y2fODiUJ1Hj9hDdQT5nJApOZsd2FMNsSBnUnKXX2M1iKbXjBqchPNF5M0AbvAOMqeE', category: 'snacks', stock: 12 },
  { id: 2, name: '現煮茶葉蛋', price: 16, desc: '秘方滷製，香嫩入味，補充蛋白好幫手。', img: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDc-5tbmzTYqAiA5fQ01bU5l4KNDsGUhIXxh1AuHgonEy0X379BVPeHXSktRsW7ppBVMnO4VXLLMkYSfprGXGDNW7_Db97Q9WwRodW_7NB2wei0wX603dUaqQbiU12iMr2t7qkqu8zXFXGyGfalFj63zMED0L-pVAbm6MT2ynfRQh1rChh9jp9MHodShfeJG5sWZwWSxKrjPQY-glgCTC1ssgt_dMvmvhfm6zIWLRhoAJb7TWuoEwKMeuufLGzXZ3Mw81nMYbh6jVQ', category: 'snacks', stock: 0 },
  { id: 3, name: '義式小披薩', price: 40, desc: '個人份現烤披薩，濃郁起司即刻享用。', img: 'https://lh3.googleusercontent.com/aida-public/AB6AXuDNaadB8hilt2bQEYiTs7M5vDoe1YqSLyn9WEEOd24n7UTnYY-_Vfh-t8y8IOkdE_EOYH5jHoqYlkCND_qfDJ_QSQF8AfGOW-uDcGHX5zYaPhYU2ob9AOQZZw10PTzsT4q-nDlqi5amT_obrptMGGHwP3eDqacJROICB6pk4acUECU76au584oMXG85lHs3_f5ruBVkl16A_jh4HRjv3PTyh3Xi5vGTLh3ywJwX9XlVLxuScvLY9NLyzW4sRCz0aSM3VPJ6T3EdVuk', category: 'snacks', stock: 5 },
];
    ...
    <div className="space-y-8 pb-6">
          ...
          <div className="bg-surface-container-lowest rounded-[2.2rem] p-7 space-y-7 group-hover:shadow-md transition-shadow relative overflow-hidden">
            <div className="flex justify-between items-start relative z-10">
              <div className="space-y-1.5">
                <span className="text-[10px] uppercase tracking-[0.25em] font-extrabold text-primary flex items-center gap-2 font-mono">
            ...
            <div className="relative pl-6 space-y-8 mt-2 z-10">
              ...
              <div className="relative flex items-center gap-5">
                ...
                <div className={`flex-1 px-5 py-3 rounded-2xl transition-all border ${orderStatus ? 'bg-surface-container-low border-outline-variant/30 text-on-surface shadow-sm' : 'opacity-40 border-transparent'}`}>
                  ...
                  {orderStatus && <p className="text-[10px] text-primary font-extrabold uppercase tracking-widest font-mono mt-1">預計抵達: 4m 20s</p>}
                </div>
              </div>
            </div>
      ...
      <section className="space-y-5 px-1">
        <div className="flex justify-between items-end px-1">
          <h2 className="text-xl font-headline font-bold tracking-tight">智能分類檢索</h2>
          <button onClick={() => { setActiveCategory('all'); setSearchQuery(''); }} className="text-[10px] text-primary font-bold hover:bg-primary/5 px-4 py-2 rounded-xl transition-all uppercase tracking-[0.15em] border border-primary/10 active:scale-95 font-mono">顯示全部</button>
        </div>
                ...
                <div className="w-[124px] h-[124px] rounded-[2rem] overflow-hidden bg-surface-container-low shrink-0 relative shadow-inner">
                  ...
                  {product.stock === 0 && (
                    <div className="absolute inset-0 bg-black/40 backdrop-blur-[2px] z-20 flex items-center justify-center">
                      <span className="text-white text-[10px] font-bold px-3 py-1.5 bg-black/50 rounded-lg uppercase tracking-widest border border-white/20">已售罄</span>
                    </div>
                  )}
                  <div className="absolute top-2 left-2 z-20">
                     <div className="bg-white/90 backdrop-blur shadow-sm px-2 py-0.5 rounded-lg text-[8px] font-bold text-primary uppercase tracking-tighter">高級</div>
                  </div>
                </div>
                <div className="flex-1 space-y-3 py-1 pr-1 min-w-0">
                  ...
                  <div className="flex items-center justify-between pt-1">
                    <div className="flex items-center gap-3">
                       <span className="font-mono font-bold text-2xl text-primary tracking-tighter">NT${product.price}</span>
                       <div className="h-4 w-[1px] bg-outline-variant/30"></div>
                       <span className={`text-[10px] font-bold uppercase tracking-widest ${product.stock > 10 ? 'text-[#87d46c]' : 'text-error'}`}>
                         庫存: {product.stock}
                       </span>
                    </div>
          ...
          <div className="p-6 space-y-10 pb-10">
            <div className="w-full aspect-square rounded-[3rem] overflow-hidden bg-surface-container mb-6 shadow-2xl relative border-4 border-surface-container-highest">
               ...
               <div className="absolute bottom-8 left-8">
                  <span className="bg-primary px-4 py-1.5 rounded-xl text-[10px] font-extrabold text-white uppercase tracking-[0.2em] shadow-lg shadow-primary/30 font-mono">品質認證</span>
               </div>
            </div>

            <div className="space-y-4 px-1">
              <div className="flex items-center gap-3">
                 <span className="bg-primary/10 text-primary px-3 py-1 rounded-lg text-[9px] font-extrabold uppercase tracking-widest leading-none border border-primary/20 font-mono italic">
                   {CATEGORIES.find(c => c.id === selectedProduct.category)?.label || 'PREMIUM'}
                 </span>
                 <div className="h-1 w-1 bg-outline-variant rounded-full"></div>
                 <span className="text-[10px] text-on-surface-variant/60 font-bold uppercase tracking-widest font-mono">現場庫存: {selectedProduct.stock}</span>
              </div>
            ...
            <div className="flex items-center justify-between border-y border-outline-variant/20 py-10 my-10 px-2">
               <div className="flex flex-col">
                 <span className="text-[10px] font-bold text-on-surface-variant/60 uppercase tracking-[0.2em] mb-2 font-mono">訂單總計</span>
                 <motion.span
                   key={qty}
                   initial={{ scale: 0.9, opacity: 0 }}
                   animate={{ scale: 1, opacity: 1 }}
                   className="font-mono font-bold text-5xl text-primary tracking-tighter"
                 >
               ...
               <div className="flex items-center gap-5 bg-surface-container rounded-[2rem] p-2.5 shadow-inner border border-outline-variant/10">
                  ...
                  <span className="font-headline font-bold text-3xl w-10 text-center text-on-surface font-mono">{qty}</span>
             ...
             <div className="mb-10 px-2">
                <label className="block text-[10px] font-extrabold text-on-surface-variant/60 uppercase tracking-[0.25em] mb-4 font-mono">選擇目的分派點</label>
            ...
            >
              ...
              {isOrdering ? (
                <div className="flex items-center gap-3">
                  ...
                  <span className="font-mono tracking-widest uppercase text-sm">訂單傳送中...</span>
                </div>
              ) : (
                <>
