...
const initialZoneState = (): ZoneState => ({
  ...
  level: 'ok',
  status: '等待影片',
  summary: '放入影片後，系統會把影片當成假即時鏡頭，持續做 YOLO + CV 人流偵測。',
  source: 'waiting',
  updatedAt: null,
  flowScore: 0,
  boxes: [],
...
export function ScanMapCard({active = true}: {active?: boolean}) {
  ...
  const [zoneStates, setZoneStates] = useState<Record<string, ZoneState>>(() =>
    Object.fromEntries(SCAN_ZONES.map(zone => [zone.id, initialZoneState()])),
  );
  ...
  const handleVideoFile = (zoneId: ZoneId, file: File | undefined) => {
    ...
    setZoneStates(prev => ({
      ...
      [zoneId]: {
        ...initialZoneState(),
        status: '影片載入中',
        summary: `${file.name} 已放入，開始作為假即時鏡頭來源。`,
        videoName: file.name,
      },
    }));
  };
  ...
  const currentState = zoneStates[currentZone.id] ?? initialZoneState();
  const hasAnyVideo = SCAN_ZONES.some(zone => Boolean(videoUrls[zone.id]));
        ...
        <div className="mb-4 flex flex-wrap items-center justify-between gap-3">
          <div>
            <div className="flex items-center gap-2">
              <span className={`h-2.5 w-2.5 rounded-full ${levelDot(currentState.level)} ${analyzingZone ? 'animate-pulse' : ''}`} />
              <p className="text-sm font-black text-white">校園三點人流監控</p>
            </div>
            <p className="mt-1 text-[10px] font-bold text-white/45">
              {hasAnyVideo ? `目前掃描：${currentZone.label} · CV ${Math.round(1000 / CV_INTERVAL_MS)}fps / YOLO 快速複核` : '三個地點可各自放入影片作為假即時鏡頭'}
            </p>
          </div>
          <div className="rounded-2xl border border-white/10 bg-white/10 px-3 py-2 text-right backdrop-blur">
            <div className="flex items-center justify-end gap-1 text-white">
              <Users size={14} />
              <span className="font-headline text-2xl font-black leading-none">{currentState.count ?? '--'}</span>
            </div>
            <p className="mt-1 text-[9px] font-bold text-white/55">{currentState.source === 'yolo' ? 'YOLO + CV' : currentState.source === 'cv' ? 'CV fallback' : '待分析'}</p>
          </div>
        </div>
