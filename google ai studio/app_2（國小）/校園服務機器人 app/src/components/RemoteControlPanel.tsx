...
export function RemoteControlPanel({isOpen, onClose}: {isOpen: boolean; onClose: () => void}) {
  ...
  const testChassis = async () => {
    ...
    if (!health.bridgeOnline || !health.arduinoConnected) {
      setLastResult({
        ok: false,
        text: health.bridgeOnline ? 'Arduino 未連線，無法測試底盤' : '硬體橋接未啟動，無法測試底盤',
      });
      setChassisTesting(false);
      return;
    }

    setLastResult({ok: true, text: '底盤測試：短暫前進中'});
    try {
      ...
      setLastResult({ok: true, text: '底盤測試完成：已停止'});
      await refreshHardwareHealth();
    } catch (error) {
      await sendHardwareCommand('STOP', 'app2:chassis-test-cleanup').catch(() => {});
      setLastResult({ok: false, text: error instanceof Error ? error.message : '底盤測試失敗'});
    } finally {
      setChassisTesting(false);
    }
  };
  ...
  const testBlades = async () => {
    setLastResult({ok: true, text: '葉片測試：M3/M4 滿速直驅中'});
    try {
      ...
      setLastResult({ok: true, text: '葉片測試完成：M3/M4 已停止'});
    } catch (error) {
      ...
      setLastResult({ok: false, text: error instanceof Error ? error.message : '葉片測試失敗'});
    }
  };
      ...
      <div className="flex flex-col gap-5 p-5 pb-8">
        ...
        <div className="flex items-center gap-3 rounded-2xl border border-outline-variant/20 bg-surface-container-low/60 p-3">
          ...
          <button
            ...
            className={`flex min-h-11 shrink-0 items-center gap-2 rounded-2xl px-3 py-2 text-left transition active:scale-95 disabled:opacity-70 ${
          ...
          >
            ...
            <span className="flex flex-col leading-tight">
              ...
              <span className="max-w-36 truncate text-[10px] font-bold opacity-75">
                {chassisTesting ? '測試底盤中...' : hardwareHealth.arduinoConnected ? '點我測試底盤' : '點我重新檢查'}
              </span>
            </span>
          </button>
        </div>
            ...
            <div className="space-y-3 rounded-2xl border border-emerald-200 bg-emerald-50/40 p-4">
              ...
              <div className="grid grid-cols-3 gap-2">
                ...
                <button
                  type="button"
                  onClick={testBlades}
                  className="flex h-12 items-center justify-center gap-2 rounded-xl bg-surface-container-lowest text-sm font-black text-on-surface-variant ring-1 ring-outline-variant/40 transition hover:ring-emerald-300 active:scale-95"
                  title="滿速短暫測試 M3/M4 葉片"
                >
