...
export function analyzeFrameQuality(width: number, height: number, data: Uint8ClampedArray | number[]): FrameQualityResult {
  ...
  const hints: string[] = [];
  if (metrics.brightness < 24 || metrics.darkArea > 42) hints.push('光線偏暗，請靠近光源或打開教室燈。');
  if (metrics.brightness > 88 || metrics.brightArea > 58) hints.push('畫面過曝，請避開窗邊強光或降低螢幕反光。');
  if (metrics.edgeDensity < 6 && metrics.contrast < 18) hints.push('畫面資訊太少，請對準白板/場域重點後再辨識。');
  if (metrics.edgeDensity < 10 && metrics.contrast >= 18) hints.push('畫面可能失焦，請穩住鏡頭再拍一次。');
  const level = hints.length === 0 ? 'good' : hints.length === 1 ? 'warn' : 'poor';

  return {
    level,
    label: level === 'good' ? '畫面品質良好' : level === 'warn' ? '畫面可用但建議調整' : '建議重新拍攝',
    hints,
    metrics,
  };
}
