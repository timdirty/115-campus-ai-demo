...
export default function TeacherDashboard() {
  ...
  const readinessChecks = useMemo(() => ([
    ...
    {
      label: '硬體路徑',
      ok: hardwareNotice.includes('已送到機器人') || hardwareNotice.includes('正在建立'),
      detail: hardwareBusy ? '任務送出中' : '未連線時會保留備援紀錄',
    },
  ...
  const saveHardwareProfile = async () => {
    ...
    try {
      ...
      setHardwareNotice('已保存實機校正設定，接上硬體後可直接使用這組角度。');
      setNotice('實機校正設定已保存');
    } catch (error) {
  ...
  const sendCalibrationPreview = async (command: string, label: string) => {
    if (hardwareBusy) {
      setHardwareNotice('上一個硬體指令仍在處理，請稍後。');
      return;
    }
    setHardwareBusy(command);
    setHardwareNotice(`正在送出 ${label} 校正指令...`);
    try {
      ...
      setHardwareNotice(result.ok
        ? `${label} 校正指令已送出：${result.response || result.command}`
        : `${label} 指令已記錄，請確認硬體橋接是否連線`);
      setNotice(result.ok ? `${label} 校正成功` : `${label} 已保留展示紀錄`);
    } catch (error) {
      setHardwareNotice(error instanceof Error ? error.message : `${label} 校正指令送出失敗`);
    } finally {
      setHardwareBusy('');
    }
  };
  ...
  const pushServoProfileToRobot = async () => {
    ...
    setHardwareNotice('正在把校正角度寫到 UNO R4...');
    try {
      for (const item of sequence) {
        await sendRobotCommand(item.command, 'teacher-calibration');
      }
      setHardwareNotice('UNO R4 已收到 A/B/C、全擦與待命角度，接著可以預覽各區域。');
      setNotice('校正角度已推送到硬體');
    } catch (error) {
      setHardwareNotice(error instanceof Error ? error.message : '無法推送校正角度到硬體');
    } finally {
      setHardwareBusy('');
    }
  };
    ...
    try {
      ...
      if (session && hardwareProfileDraft) {
        const next = await saveClassroomSession({
          hardwareProfile: {
            ...hardwareProfileDraft,
            robotPose: estimateRobotPose(result.command, {
              boardRegions: session.boardRegions,
              boardCalibration: hardwareProfileDraft.boardCalibration,
              servoAngles: hardwareProfileDraft.servoAngles,
              previousPose: session.hardwareProfile.robotPose,
      ...
      if (result.ok && action === 'erase' && regionId) {
        setCompletedRegions((prev) => prev.includes(regionId) ? prev : [...prev, regionId]);
      } else if (result.ok && action === 'erase' && !regionId) {
        // erase_all case
        setCompletedRegions(session.boardRegions.map((r) => r.id));
      }
    ...
    const cancel = eraseRegionSequence(targets, (event: EraseSequenceEvent) => {
      ...
      if (event.type === 'done') {
        ...
        setNotice(event.failed === 0
          ...
          : `完成 ${event.ok} 個，${event.failed} 個未確認（硬體未連線時為正常）`);
        ...
        {!session ? (
          ...
          <div className="grid grid-cols-1 xl:grid-cols-12 gap-5 sm:gap-6">
              ...
              {session.boardOcrText && (
                <div className="mt-5 rounded-2xl border border-primary/15 bg-surface p-4">
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <p className="text-xs font-black text-primary">白板 OCR 辨識文字</p>
                  </div>
            ...
            <motion.section variants={itemVariants} className="xl:col-span-3 bg-surface-container-lowest rounded-3xl p-5 sm:p-7 border border-outline-variant/10 shadow-premium">
              ...
              {hardwareProfileDraft && (
                <div className="mb-5 rounded-2xl border border-outline-variant/10 bg-surface p-4">
                  <div className="flex items-start justify-between gap-3">
                    <div>
                      <p className="text-xs font-black text-primary">實機校正</p>
                      <h3 className="mt-1 text-lg font-extrabold">攝影機與板擦角度</h3>
                      <p className="mt-1 text-xs font-bold leading-5 text-on-surface-variant">先把相機固定在白板前方，再逐區調整 A/B/C、全擦與待命角度。</p>
                    </div>
