    ...
    <section className="lg:col-span-2 bg-surface-container-low rounded-lg p-5 border border-outline-variant/20">
      ...
      <div className="flex flex-col sm:flex-row gap-4">
        ...
        <div className="flex-1 min-w-0">
          <p className="text-xs font-bold text-primary">{latestNote?.subject ?? '尚無紀錄'}</p>
          <h3 className="text-2xl font-extrabold mt-1 break-words line-clamp-2">{latestNote?.title ?? '請先拍白板或新增一筆課堂紀錄'}</h3>
          <p className="text-sm text-on-surface-variant mt-2 line-clamp-3" title={latestNote?.desc}>{latestNote?.desc ?? '保存後會出現在課堂紀錄本、AI 小老師與學習單生成器。'}</p>
