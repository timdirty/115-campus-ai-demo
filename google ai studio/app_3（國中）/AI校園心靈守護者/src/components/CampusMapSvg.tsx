...
function SensorBadge({sensor, x, y}: {sensor: ZoneSensorReading | undefined; x: number; y: number}) {
  ...
  return (
    <g>
      ...
      <text x={x + 4} y={y + 9} fontSize="7.5" fill="#fcd34d" fontWeight="700">T:{temp}</text>
      <text x={x + 4} y={y + 19} fontSize="7.5" fill="#7dd3fc" fontWeight="600">H:{hum}</text>
      {/* breathing live dot */}
      <circle cx={x + 46} cy={y + 11} r="2.5" fill="#34d399">
...
function SelectionRing({x, y, w, h, rx}: {x: number; y: number; w: number; h: number; rx: number}) {
  return (
    <rect
      x={x - 2}
      y={y - 2}
  ...
  return (
    ...
    >
      ...
      <g aria-label="圖書館" className="zone-clickable" onClick={() => zoneClick('zone-library')}>
        ...
        <SensorBadge sensor={libSensor} x={16} y={52} />
        {selectedZoneId === 'zone-library' && <SelectionRing x={10} y={10} w={100} h={70} rx={8} />}
      </g>
      ...
      <g aria-label="穿堂" className="zone-clickable" onClick={() => zoneClick('zone-hall')}>
        ...
        <SensorBadge sensor={hallSensor} x={124} y={52} />
        {selectedZoneId === 'zone-hall' && <SelectionRing x={120} y={10} w={80} h={70} rx={8} />}
      </g>
      ...
      <g aria-label="操場" className="zone-clickable" onClick={() => zoneClick('zone-field')}>
        ...
        {selectedZoneId === 'zone-field' && <SelectionRing x={300} y={10} w={90} h={200} rx={8} />}
      </g>
      ...
      <g aria-label="九年級教室" className="zone-clickable" onClick={() => zoneClick('zone-classroom')}>
        ...
        <SensorBadge sensor={classroomSensor} x={104} y={162} />
        {selectedZoneId === 'zone-classroom' && <SelectionRing x={10} y={118} w={150} h={80} rx={8} />}
      </g>
      ...
      <g aria-label="體育館" className="zone-clickable" onClick={() => zoneClick('zone-gym')}>
        ...
        <SensorBadge sensor={gymSensor} x={174} y={168} />
        {selectedZoneId === 'zone-gym' && <SelectionRing x={170} y={118} w={120} h={80} rx={8} />}
      </g>
