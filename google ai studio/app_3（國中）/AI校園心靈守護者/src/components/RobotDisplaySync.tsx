...
interface Props { latestMood?: MoodType; alertCount?: number; }

const BRIDGE_URL = 'http://localhost:3203';

export const RobotDisplaySync = memo(function RobotDisplaySync({latestMood, alertCount = 0}: Props) {
  ...
  const generateQr = useCallback(async () => {
    ...
    try {
      ...
      const info = await infoRes.json() as {bridgePort?: number; ip?: string; vitePort?: number; robotDisplayUrl: string};
      ...
      const displayUrl = info.robotDisplayUrl
          ...
          <div className="rounded-xl bg-white/80 ring-1 ring-black/5 overflow-hidden">
            <div className="flex items-center justify-between px-3 py-2 border-b border-black/5">
              <div className="flex items-center gap-1.5">
                ...
                <span className="text-[9px] font-black tracking-[0.2em] uppercase text-stone-500">iPad 掃碼直接開啟</span>
              </div>
              <button
                type="button"
                onClick={generateQr}
            ...
            {qrSrc ? (
              <div className="flex flex-col items-center gap-2 p-3">
                <img src={qrSrc} alt="Robot Display QR Code" className="h-44 w-44 rounded-lg shadow-sm" />
              ...
              <div className="px-3 py-4 text-center">
                <p className="text-[10px] text-stone-500">QR 碼產生中</p>
