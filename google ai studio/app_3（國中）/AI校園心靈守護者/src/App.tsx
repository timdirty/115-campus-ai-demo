...
type TeacherCallDialog = {zoneId: string; zoneName: string; location: string; riskLevel: Exclude<RiskLevel, 'low'>; createdAt: number} | null;
...
function getZoneStatusLabel(zone: SchoolZoneStatus) {
...
function getZoneTone(zone: SchoolZoneStatus) {
...
function AppContent() {
  ...
  const [teacherCallDialog, setTeacherCallDialog] = useState<TeacherCallDialog>(null);
  ...
  useEffect(() => {
    if (setupPromptShownRef.current || showSetup) return;
    if (detectedPorts.length > 0 && detectedPorts.some((port) => !port.assignedZone)) {
      setupPromptShownRef.current = true;
      setShowSetup(true);
    }
  }, [detectedPorts, showSetup]);
  ...
  useEffect(() => {
    if (!teacherCallDialog) return;
    const timer = window.setTimeout(() => setTeacherCallDialog(null), 4200);
    return () => window.clearTimeout(timer);
  }, [teacherCallDialog]);
  ...
  const selectZoneForRobotDisplay = useCallback((zone: SchoolZoneStatus): boolean => {
    if (teacherCalledZoneRef.current[zone.id]) {
      showToast(`${zone.name}已呼叫師長，等數值正常後才恢復派遣`);
      return false;
    }
  ...
  const queueAutoDispatch = useCallback((zone: SchoolZoneStatus, reason: string, riskLevelOverride?: Exclude<RiskLevel, 'low'>) => {
    ...
    if (!riskLevel) return;
    if (teacherCalledZoneRef.current[zone.id]) return;
  ...
  const handleRobotEmotionEvent = useCallback((event: RobotEmotionEvent) => {
    ...
    if (event.source === 'robot-arrival-prompt' || event.emotion === 'incident_resolved' || event.emotion === 'teacher_called') {
      ...
      setAutoDispatchQueue((current) => current.filter((item) => item.zoneId !== zone.id));
      ...
      showToast(`${zone.name}已處理，15 秒內暫不重複派遣`);
      window.setTimeout(() => {
        if ((zoneActionCooldownRef.current[zone.id] ?? 0) > Date.now()) return;
        ...
        if (!teacherCalledZoneRef.current[zone.id]) {
          void (async () => {
            ...
            const summary = `${latestZone.name}冷卻 15 秒後仍為${getRiskStatusLabel(riskLevel)}，已直接呼叫師長。`;
            teacherCalledZoneRef.current[zone.id] = true;
            setTeacherCalledZones((current) => ({...current, [zone.id]: true}));
            setTeacherCallDialog({zoneId: latestZone.id, zoneName: latestZone.name, location: latestZone.location, riskLevel, createdAt: Date.now()});
            dispatch({
              type: 'CREATE_CONTEXT_ALERT',
              payload: {
            ...
            void pushRobotEmotionEvent({
              zoneId: latestZone.id,
              zoneName: latestZone.name,
              location: latestZone.location,
            ...
            showToast(`${latestZone.name}狀態未解除，已直接呼叫師長`);
          })();
        }
      }, 15200);
      return;
  ...
  useEffect(() => {
    for (const zone of viewModel.zones) {
      ...
      if ((zoneActionCooldownRef.current[zone.id] ?? 0) > Date.now()) {
        delete autoDispatchSeenRef.current[zone.id];
        continue;
      }
      if (teacherCalledZoneRef.current[zone.id]) {
  ...
  }, [viewModel.zones, queueAutoDispatch, teacherCalledZones]);
  ...
  return (
    <div className="guardian-shell min-h-screen overflow-x-hidden bg-[linear-gradient(160deg,#f5f9fc_0%,#eef3f8_60%,#e8f0f7_100%)] text-slate-950">
      ...
      <TeacherCallDialogNotice notice={teacherCallDialog} />
        ...
        <div className="mx-auto flex max-w-7xl items-center justify-between gap-3 px-4 py-3 sm:px-6">
          ...
          <div className="flex items-center gap-2">
            ...
            >
              ...
              <span className="hidden sm:inline">前端配對</span>
            ...
            <IconButton
              onClick={async () => {
                ...
                setDetectedPorts(ports);
                setDrivePorts(drive.ports);
                setZoneSensors(await fetchZoneSensors());
                showToast('展示資料已重置');
              }}
              label="重設展示資料"
              icon={RefreshCw}
      ...
      <main className="mx-auto grid max-w-7xl gap-4 px-4 py-4 pb-28 sm:px-6 lg:pb-20 lg:grid-cols-[minmax(0,1fr)_22rem]">
          <CommandCenterScreen
            ...
            onMoveToZone={selectZoneForRobotDisplay}
...
function CommandCenterScreen({
  ...
  onMoveToZone: (zone: SchoolZoneStatus) => void;
    ...
    <section className="grid gap-4 lg:min-h-[calc(100vh-6.5rem)] lg:grid-rows-[auto_minmax(0,1fr)_auto]">
      <div data-tour="signal-overview">
        <div className="overflow-hidden rounded-2xl border border-slate-200/60 bg-linear-to-br from-white to-teal-50/40 shadow-sm">
          <div className="p-4 sm:p-5">
            <div className="flex flex-col gap-4 xl:flex-row xl:items-center xl:justify-between">
              ...
              <div className="grid grid-cols-2 gap-2 text-center sm:min-w-[16rem]">
                <SignalTile label="最高風險" value={getZoneStatusLabel(viewModel.highestZone)} tone={isZoneIdle(viewModel.highestZone) ? 'slate' : viewModel.highestZone.riskLevel === 'high' ? 'rose' : viewModel.highestZone.riskLevel === 'medium' ? 'amber' : 'emerald'} />
          ...
          <div className={`h-1 w-full ${isZoneIdle(viewModel.highestZone) ? 'bg-slate-300' : viewModel.highestZone.riskLevel === 'high' ? 'bg-linear-to-r from-rose-400 to-rose-600' : viewModel.highestZone.riskLevel === 'medium' ? 'bg-linear-to-r from-amber-300 to-amber-500' : 'bg-linear-to-r from-teal-300 to-teal-500'}`} />
        </div>
      </div>

      <div data-tour="campus-map">
        <CampusMap2D zones={viewModel.zones} selectedZone={selectedZone} selectedZoneId={selectedZoneId} robotMapZone={robotMapZone} robotFeedback={robotFeedback} robotTravel={robotTravel} zoneAssessments={zoneAssessments} teacherCalledZones={teacherCalledZones} zoneCooldowns={zoneCooldowns} cooldownNow={cooldownNow} onSelectZone={onSelectZone} onMoveToZone={onMoveToZone} onOpenZoneInsight={onOpenZoneInsight} onDispatchRobot={onDispatchRobot} />
      </div>
        ...
        <div data-tour="dispatch-robot">
          <Surface className="h-full p-4">
            ...
            <div className="mt-4 grid grid-cols-3 gap-2">
              <MetricTile label="燈號" value={getZoneStatusLabel(viewModel.highestZone)} />
              <MetricTile label="感測" value={getZoneSensorLabel(viewModel.highestZone)} />
              <MetricTile label="提醒" value={viewModel.highestZone.alertCount} />
            </div>
            <PrimaryAction
              onClick={() => onDispatchRobot(viewModel.highestZone)}
              disabled={isZoneIdle(viewModel.highestZone) || viewModel.highestZone.riskLevel === 'low' || Boolean(robotTravel) || teacherCalledZones[viewModel.highestZone.id]}
            ...
            >
              ...
              {isZoneIdle(viewModel.highestZone) ? '等待指派感測器' : teacherCalledZones[viewModel.highestZone.id] ? '已呼叫師長' : robotTravel ? '機器人移動中' : viewModel.highestZone.riskLevel === 'low' ? '維持一般巡查' : robotFeedback?.zoneId === viewModel.highestZone.id ? '已送出派遣' : '派遣機器人介入'}
            </PrimaryAction>
          </Surface>
        </div>
      </div>
...
function CampusMap2D({
  ...
  const travelColor = selectedIdle ? '#94a3b8' : getRiskStatusColor(robotTravel?.riskLevel ?? selectedZone.riskLevel);
        ...
        <div className="absolute bottom-3 left-3 right-3 z-30 rounded-2xl border border-white/60 bg-white/88 p-3 shadow-xl shadow-slate-300/30 backdrop-blur-md">
          <div className="grid gap-3 sm:grid-cols-[1fr_auto_auto] sm:items-center">
            <div>
              ...
              <p className="font-black text-slate-950">{selectedZone.name} · {getZoneStatusLabel(selectedZone)}</p>
            ...
            {selectedIdle ? <IdleChip /> : <StatusChip level={selectedZone.riskLevel} />}
            <button
            ...
            >
              {selectedIdle ? '待機' : teacherCalledZones[selectedZone.id] ? '已呼叫師長' : mapLocked ? '移動中' : selectedZone.riskLevel === 'low' ? '維持巡查' : selectedDispatch ? robotFeedback?.stage : '派遣'}
            </button>
          </div>
          {selectedDispatch && (
            <motion.div
    ...
    <div className="mb-3 overflow-hidden rounded-2xl border border-slate-200/80 bg-white shadow-sm">
      ...
      <div className="grid gap-2 md:grid-cols-3">
        {zones.map((zone) => {
          ...
          const cardBorder = idle ? 'border-slate-200' : zone.riskLevel === 'high' ? 'border-rose-300' : zone.riskLevel === 'medium' ? 'border-amber-300' : 'border-emerald-300';
          const tone = getZoneTone(zone);
          const statusLabel = dispatching ? '派遣中' : getZoneStatusLabel(zone);
          const sensor = zone.sensor;
          const sourceLabel = idle ? '未指派' : !assessment ? '待判讀' : assessment.source === 'fallback' ? '備援規則' : 'LLM 判讀';
          ...
          return (
            <button
              ...
              onClick={() => {
                onSelectZone(zone);
                if (!idle) onOpenZoneInsight(zone);
              }}
            ...
            >
              ...
              <div className="mt-3 grid gap-2 sm:grid-cols-[0.9fr_1.1fr] sm:items-stretch">
                <div className={`flex min-h-16 flex-col justify-center rounded-xl border px-3 py-2 ${tone.panel}`}>
                  ...
                  <p className="mt-1 text-[10px] font-black text-slate-500">{idle ? '等待感測器位置指派' : '依 LLM 對感測器數值判斷'}</p>
      ...
      >
        ...
        <div className="p-5">
          <div className="flex items-start justify-between gap-4">
            <div>
              <p className="text-xs font-black tracking-wide text-teal-600">Gemini 區域判讀</p>
          ...
          <div className="mt-4 rounded-2xl bg-slate-50 p-4">
            {loading ? (
              <div className="flex min-h-36 flex-col items-center justify-center gap-3 text-slate-500">
                ...
                <p className="text-sm font-black">正在請 Gemini 判讀區域狀況...</p>
              </div>
            ) : (
              <div className="grid gap-4">
                ...
                <p className="text-[10px] font-bold text-slate-400">
                  來源：{result?.source === 'google-ai-studio' ? `Google AI Studio / ${result.model ?? 'Gemini'}` : result?.source === 'ollama-gemma' ? `Ollama / ${result.model ?? 'Gemma'}` : result?.source === 'cloud-gemma' ? `Cloud / ${result.model ?? 'Gemma'}` : '本機備援判讀'}
                </p>
              </div>
            )}
          </div>
  ...
  return (
    <Surface className="p-4">
      <div className="flex items-start justify-between gap-3">
        ...
        {idle ? <IdleChip /> : <StatusChip level={zone.riskLevel} />}
      </div>
      <div className={`mt-4 overflow-hidden rounded-2xl border ${tone.panel}`}>
        ...
        <div className="flex items-center justify-between gap-3 p-4">
          <div>
            ...
            <p className={`mt-1 text-3xl font-black ${tone.text}`}>{getZoneStatusLabel(zone)}</p>
          </div>
          <span className={`h-6 w-6 rounded-full shadow-sm ${tone.dot}`} />
        </div>
      </div>
      <div className="mt-4 grid grid-cols-3 gap-2">
        <MetricTile label="燈號" value={getZoneStatusLabel(zone)} />
      ...
      <div className="mt-4 rounded-xl border border-teal-100 bg-teal-50 p-3">
        ...
        <p className="mt-1 text-sm font-bold text-teal-900">{idle ? '請先指派感測器位置' : activeDispatch ? robotFeedback?.stage : zone.riskLevel === 'low' ? '維持巡查' : '派遣 + 確認'}</p>
      </div>
      {activeDispatch && (
      ...
      <PrimaryAction onClick={() => onDispatchRobot(zone)} disabled={idle || zone.riskLevel === 'low' || activeDispatch} active={activeDispatch} className="mt-4">
        <Bot className={`h-5 w-5 ${activeDispatch ? 'animate-pulse' : ''}`} />
        {idle ? '區域待機' : zone.riskLevel === 'low' ? '維持巡查' : activeDispatch ? '已送出派遣' : '指派機器人'}
      </PrimaryAction>
    </Surface>
  );
}
      ...
      <GlassPanel>
        ...
        <div className="mt-4 space-y-2">
          {zones.map((zone) => (
            <button
              key={zone.id}
              onClick={() => !isZoneIdle(zone) && zone.riskLevel !== 'low' && onDispatchRobot(zone)}
              disabled={isZoneIdle(zone) || zone.riskLevel === 'low' || robotFeedback?.zoneId === zone.id}
...
function FrontendPairingModal({
  ...
  const generatePairingQr = useCallback(async () => {
    ...
    try {
      ...
      const displayPort = window.location.protocol === 'https:'
        ? String(info?.vitePort || window.location.port || 3001)
        : window.location.port === '3000' ? '3001' : window.location.port;
      const displayHost = isLocalHost && info?.ip
        ? `${info.ip}${displayPort ? `:${displayPort}` : ''}`
        : window.location.host;
      const url = info?.robotDisplayUrl
        ? `${info.robotDisplayUrl}?pair=${Date.now().toString(36)}`
        : `https://${displayHost}/robot-display.html?pair=${Date.now().toString(36)}`;
      setPairingUrl(url);
              ...
              <section className="border-b border-slate-200 p-5 lg:border-b-0 lg:border-r">
                ...
                <div className="flex flex-col items-center rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
                  ...
                  <p className="mt-2 rounded-xl bg-amber-50 px-3 py-2 text-center text-[11px] font-black leading-5 text-amber-700 ring-1 ring-amber-200">
                    iPad 網址必須包含 :3443；如果看到 :3203、:3001 或沒有 :3443，請重新掃新的 QR。
                  </p>
...
function TeacherCalledText({zoneId, teacherCalledZones, compact = false}: {zoneId: string; teacherCalledZones: Record<string, boolean>; compact?: boolean}) {
  ...
  return (
    <p className={`${compact ? 'mt-0.5 text-[10px]' : 'mt-1 text-xs'} font-black text-rose-600`}>
      已呼叫師長
    </p>
  );
}

function TeacherCallDialogNotice({notice}: {notice: TeacherCallDialog}) {
  return (
    <AnimatePresence>
      {notice && (
        <motion.div
        ...
        >
          <motion.div
            ...
            className="w-full max-w-md rounded-3xl border border-amber-200 bg-white p-6 text-center shadow-2xl"
          >
            <div className="mx-auto flex h-14 w-14 items-center justify-center rounded-2xl bg-amber-100 text-amber-700">
              <Siren className="h-7 w-7" />
            </div>
            <p className="mt-4 text-xs font-black tracking-widest text-amber-600 uppercase">狀態未解除</p>
            <h2 className="mt-2 text-3xl font-black text-slate-950">已呼叫師長</h2>
            <p className="mt-3 text-sm font-bold leading-6 text-slate-600">
              {notice.zoneName} 冷卻後仍為{getRiskStatusLabel(notice.riskLevel)}，中控已通知師長到場確認。
            </p>
          </motion.div>
        </motion.div>
      )}
...
function SignalTile({label, value, tone}: {label: string; value: string; tone: 'teal' | 'rose' | 'amber' | 'emerald' | 'slate'}) {
