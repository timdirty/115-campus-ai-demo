...
import {
  ...
  Loader2, AlertCircle, RefreshCw, MapPin, Bot, CheckCircle2, UserRoundCheck
} from 'lucide-react';
...
const shouldShowArrivalPrompt = (assignment, teacherCalledZones) => (
  ...
  && !/已呼叫師長/.test(`${assignment.statusLabel || ''} ${assignment.stage || ''}`)
);
...
const Drawer = ({ open, onClose, position, children }) => {
  ...
  return (
    <aside className={`${baseCls} ${posCls} robot-drawer robot-drawer-${position}`} style={transformStyle}>
...
export default function App() {
  ...
  const requestMediaPermissions = useCallback(async () => {
    ...
    showBubble('請在瀏覽器允許相機與麥克風權限，再重新啟用。', 6200);
  ...
  const closeArrivalPrompt = useCallback((action) => {
    ...
    _setBubble(`${prompt.zoneName}：${actionText}`, 4200);
    fetch(`${getBridgeHttpUrl(bridgeInput)}/api/display/emotion-event`, {
      method: 'POST',
      headers: {'Content-Type': 'application/json'},
      body: JSON.stringify({
        ...
        description: action === 'teacher'
          ? `機器人已在 ${prompt.zoneName} 呼叫師長協助確認。`
...
bcHandlerRef.current = (data) => {
  ...
  useEffect(() => {
    ...
    const hit = (element, point, pad = 18) => {
    ...
    const handler = (event) => {
      if (arrivalPromptRef.current) return;
      const point = pointFromEvent(event);
      if (!point) return;
      if (hit(scanButtonRef.current, point, 24)) {
        runTouchAction(event, startScan);
        return;
      }
      if (hit(listenButtonRef.current, point, 22)) {
        runTouchAction(event, toggleListen);
        return;
      }
      if (hit(introButtonRef.current, point, 22)) {
        runTouchAction(event, () => introPlaying ? cancelIntro() : playIntro());
        return;
      }
      if (hit(cameraButtonRef.current, point, 18)) {
        runTouchAction(event, toggleCamera);
        return;
      }
      if (hit(permissionButtonRef.current, point, 22)) {
  ...
  return (
    ...
    >
      {arrivalPrompt && (
        ...
        >
          ...
          >
            ...
            <p style={{margin: '18px auto 0', maxWidth: 430, fontSize: 17, lineHeight: 1.7, fontWeight: 700, color: '#334155'}}>
              請先確認現場事件是否解除，或呼叫師長協助。完成選擇前，機器人前端會暫停其他操作。
            </p>
            <div style={{display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 14, marginTop: 26}}>
              ...
              <button
                ...
                style={{
                  ...
                  justifyContent: 'center',
                  gap: 10,
                  boxShadow: '0 14px 28px rgba(249,115,22,0.3)',
                  touchAction: 'manipulation',
                }}
              >
            ...
            <p style={{margin: '14px 0 0', fontSize: 12, fontWeight: 700, color: '#94a3b8'}}>
              若中控派出高風險區域任務，會自動優先切換。
            </p>
          </div>
        </div>
      )}
      ...
      <div className="robot-bridge-settings" style={{position:'fixed',top:12,right:12,zIndex:9999,display:'flex',flexDirection:'column',alignItems:'flex-end',gap:8}}>
        <button
      ...
      {(!cameraReady || !ambientAudio.active || mediaPermission === 'blocked') && !movementRoute && (
        <div className="robot-permission-card absolute left-1/2 z-[80] w-[min(88vw,520px)] -translate-x-1/2 rounded-3xl border border-cyan-200/70 bg-white/92 p-3 shadow-2xl shadow-slate-900/10 backdrop-blur-xl">
          <div className="flex items-center gap-3">
            <div className="flex h-12 w-12 shrink-0 items-center justify-center rounded-2xl bg-cyan-50 text-cyan-700">
              {mediaPermission === 'requesting' ? <Loader2 className="h-6 w-6 animate-spin" /> : <Camera className="h-6 w-6" />}
            </div>
            <div className="min-w-0 flex-1">
              <p className="text-sm font-black text-slate-900">啟用鏡頭與麥克風</p>
              <p className="mt-0.5 truncate text-xs font-bold text-slate-500">
                {mediaAccessIssue || '情緒偵測需要手機瀏覽器允許相機與麥克風權限'}
              </p>
            </div>
            <button
          ...
          {(mediaPermission === 'blocked' || mediaAccessIssue) && (
            <p className="mt-2 rounded-2xl bg-amber-50 px-3 py-2 text-xs font-bold leading-5 text-amber-800">
              {mediaAccessIssue
                ? '目前網址不是安全來源，所以瀏覽器不會顯示權限視窗。請改用 HTTPS/tunnel 網址後再按允許。'
                : '若沒有跳出權限視窗，請到 Safari/Chrome 網站設定允許相機與麥克風，或改用 HTTPS/tunnel 開啟。'}
            </p>
          )}
        </div>
      )}
