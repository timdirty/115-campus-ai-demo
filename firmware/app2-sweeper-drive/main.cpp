...
//   SPEED:<0-255>            設定底盤速度
//   SWEEP_START / SWEEP_STOP 滾筒啟停
//   SWEEP_REVERSE            滾筒方向切換（吸入 ↔ 推出）
//   SWEEP_SPEED:<0-255>      滾筒速度
...
constexpr bool invertMotor1 = true;   // M1 左輪（接線反）
constexpr bool invertMotor2 = true;   // M2 右輪（接線反）
...
void printReadyMessage() {
  ...
  Serial.println("Wiring: M1=left wheel, M2=right wheel, M3+M4=sweeper rollers.");
  Serial.println("Drive: FORWARD/BACKWARD/LEFT/RIGHT/STOP, SPEED:<0-255>");
  Serial.println("Sweep: SWEEP_START/SWEEP_STOP/SWEEP_REVERSE, SWEEP_SPEED:<0-255>");
...
void handleCommand(const String &command) {
  ...
  // ── Sweep primitives ─────────────────────────────────────────────────────
  if (command == "SWEEP_START") {
    sweepStart();
    printState("SWEEP_START");
    return;
  }
  if (command == "SWEEP_STOP") {
    sweepStop();
    printState("SWEEP_STOP");
    return;
  }
  if (command == "SWEEP_REVERSE") {
    sweepToggleDirection();
    printState("SWEEP_REVERSE");
    return;
  }
  if (parsePrefixedSpeed(command, "SWEEP_SPEED:", requestedSpeed)) {
    sweepSpeed = requestedSpeed;
    if (sweepRunning) applySweep();
    printState("SWEEP_SPEED_SET");
    return;
  }
